...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  const call = await createCall({ to, from, url: `${SITE_URL}/api/twilio/receptionist/test-entry?version=${version}${voice ? `&voice=${encodeURIComponent(voice)}` : ''}`, statusCallback: `${SITE_URL}/api/twilio/status` });
  console.log(`[receptionist-test] ${gate.email} placed test call ${call.sid} to ${to} (${version}, voice ${voice ?? 'default'})`);
  return NextResponse.json({ callSid: call.sid, status: call.status, to, version, voice });
