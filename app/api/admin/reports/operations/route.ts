...
export async function GET(req: NextRequest): Promise<NextResponse<ReportPayload | { error: string }>> {
  ...
  const orgId: string = user.default_org_id;
  ...
  // ── Section 1: Jobs ────────────────────────────────────────────────────
  // We look at every job whose lifecycle overlaps the window: started
  // OR delivered OR result_set in [from, to]. The counts use the
  // `result` field where it's set, falling back to stage.
  const jobsSection = await loadJobs(orgId, fromIso, toIso, warnings);

  // ── Section 2: Employee Hours ─────────────────────────────────────────
...
async function loadJobs(orgId: string, fromIso: string, toIso: string, warnings: string[]) {
  ...
  // Outstanding = invoiced but not paid. We don't have a payment-status
  // column today; treat outstanding as the delta between quote total
  // and final-amount-billed for completed jobs (rough proxy until
  // an invoice-paid field lands).
  const outstandingCents = Math.max(0, invoicedTotalCents - 0);

  return {
    started, inProgress, notStarted, completed, lost, abandoned,
    quotedTotalCents, invoicedTotalCents, outstandingCents,
    details,
  };
}
...
async function loadHours(orgId: string, fromIso: string, toIso: string, warnings: string[]) {
  ...
  // Sum minutes per employee per week — OT is per-week, not per-period.
...
async function loadReceipts(orgId: string, fromIso: string, toIso: string, warnings: string[]) {
  ...
  for (const r of (data ?? []) as Row[]) {
    ...
    if (r.status in byStatus) {
      byStatus[r.status as keyof typeof byStatus] += cents;
    }
    const cat = r.category ?? 'uncategorized';
    byCategory[cat] = (byCategory[cat] ?? 0) + cents;
    if (r.user_id) {
      byEmployeeMap.set(r.user_id, (byEmployeeMap.get(r.user_id) ?? 0) + cents);
    }
    entries.push({
      id: r.id,
  ...
  // Resolve user_ids → employee_profiles.user_name for the byEmployee block
...
function isoWeekKey(d: Date): string {
  // Returns `YYYY-Www` per ISO 8601. Used to bucket time entries per week
  // for OT calculation.
