...
const SYSTEM_PROMPT = [
  ...
  'UNDERSTAND rules, feats, abilities, classes and conditions.',
  '',
  'Rules of engagement:',
...
export async function POST(req: NextRequest) {
  ...
  const focus = normalizeSystem(body?.system);
  ...
  try {
    ...
    const user = [grounding?.block || null, priorTurns ? `Conversation so far:\n${priorTurns}` : null, `Question: ${question}`]
      .filter(Boolean)
      .join('\n\n');
