...
//
// ── THE DOOR CAME TO THE BOUNDARY, WHICH IS C10's MOVE AND NOT A NARROWING ──────────────────
//
// The registry row offered this page to admin, developer and tech_support, and middleware gated
// the prefix to the same three. The API does not agree and never has: GET /api/admin/team admits
// isAdmin OR tech_support and answers 401 to everyone else, and /api/admin/team/[email]/today
// does the same. So a `developer` has always been offered a page that refuses them everything.
//
// Fourth time this plan has met a door wider than its boundary, and the answer is C10's: bring
// the door to the boundary rather than move the boundary, because who may see the crew list is a
// product decision and narrowing a door nobody could use through takes nothing from anybody. The
// tab carries [admin, tech_support] and the middleware entry matches.
//
// One endpoint under this prefix is deliberately open and was NOT touched: /api/admin/team/status
// answers any signed-in account, because three Hub widgets — daily-briefing, field-pulse and
// team-status — render it on /admin/me, which everybody has. Checked before assuming a fourth
// hole; not every 401-only route is one.
