// app/admin/support/_tabs/AuditTab.tsx — a tab of the System portal.
//
// C12a / P15. Was `/admin/audit/page.tsx`; the old route stays and forwards.
//
// Same shape as the error log and worth stating separately, because "200" looked alarming until
// somebody read the handler: `GET /api/admin/audit` answers anybody signed in, then looks up the
// caller's membership and returns `{ rows: [] }` unless they are an admin OF THE ORG. The
// protection is a row filter, not a status code. The tab still carries the registry row's three
// roles, because being offered a tab that can only ever be empty is its own small lie.
'use client';
    ...
    <div className="audit-page">
      <header>
        ...
        <nav className="audit-elsewhere" aria-label="Other history surfaces">
          ...
          {/* C12a: was `/admin/error-log`, which is a redirect into this very portal now — so the
            * link worked but took two hops to land on the tab beside this one. Pointed straight at
            * the tab. Not a self-link: it is a different surface, and this nav exists precisely to
            * send people to the log that answers their question. */}
          <Link href="/admin/support?tab=error-log">Error log</Link>
