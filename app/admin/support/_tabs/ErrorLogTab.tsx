// app/admin/support/_tabs/ErrorLogTab.tsx — a tab of the System portal.
//
// C12a / P15. Was `/admin/error-log/page.tsx`; the old route stays and forwards.
//
// ── THE DOOR MOVED, AND THE DATA WAS ALREADY SCOPED ─────────────────────────────────────────
//
// `/admin/error-log` had its OWN middleware entry — admin, developer, tech_support — and
// `/admin/support` has none. As a tab it is behind the portal's tab list rather than a prefix
// gate, which §5 only allows because the boundary is elsewhere and holds. Measured rather than
// assumed, and the measurement is more interesting than a 403 would have been: `GET
// /api/admin/errors` answers 200 to anybody signed in AND filters to `user_email` unless the
// caller is an admin asking for the admin view. Non-admins have always been able to read their
// own error reports; what they could not do is reach the page that draws them.
//
// So the tab carries the three roles the registry row had, and the endpoint keeps deciding what
// is in the rows. A tab list that hides and an endpoint that scopes are different jobs, and this
// is one of the few places both are already doing theirs.
...
import ErrorBudgetBanner from '../../error-log/ErrorBudgetBanner';
