                ...
                <table className="research-analysis-summary__table">
                  ...
                  <tbody>
                    {summary.calls.map((call, i) => (
                      <tr key={i}>
                        <td style={{ color: 'var(--theme-fg-secondary, #4B5563)', fontWeight: 500 }}>{call.seq}</td>
                        <td style={{ fontFamily: 'monospace', color: 'var(--theme-fg-primary, #111827)' }}>{call.bearing || '\u2014'}</td>
                        <td style={{ fontFamily: 'monospace', color: 'var(--theme-fg-primary, #111827)' }}>{call.distance || '\u2014'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function StatChip({ label, value, warn }: { label: string; value: number | string; warn?: boolean }) {
  return (
    <div style={{
      ...
      <span style={{ color: 'var(--theme-fg-secondary, #374151)' }}>{label}: </span>
