...
function ConfidenceBadge({ level, notes }: { level: number; notes: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', background: '#F9FAFB', borderRadius: 8, border: '1px solid #E5E7EB', marginBottom: '1.25rem' }}>
      ...
      <span style={{ color: 'var(--theme-fg-secondary, #6B7280)', fontSize: '0.82rem', flex: 1 }}>{notes}</span>
    </div>
  );
}
...
function SectionHeader({ title, icon }: { title: string; icon: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem', paddingBottom: '0.5rem', borderBottom: '2px solid #E5E7EB' }}>
      ...
      <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: 'var(--theme-fg-primary, #111827)' }}>{title}</h3>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        {item.why && !item.done && (
          <div style={{ marginTop: '0.25rem', fontSize: '0.8rem', color: 'var(--theme-fg-secondary, #6B7280)', fontStyle: 'italic' }}>
            Why: {item.why}
          </div>
        )}
      </div>
    ...
    <div>
      {sources.map((s, i) => (
        <div key={i} style={{ padding: '0.5rem 0', borderBottom: '1px solid #F3F4F6', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
          <span style={{ color: 'var(--theme-fg-muted, #9CA3AF)', fontSize: '0.8rem', flexShrink: 0, marginTop: 2 }}>🔗</span>
          <div>
            <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--theme-fg-primary, #111827)' }}>
              {s.url ? (
            ...
            {s.data_obtained && (
              <div style={{ fontSize: '0.8rem', color: 'var(--theme-fg-secondary, #6B7280)' }}>{s.data_obtained}</div>
            )}
          </div>
        </div>
      ))}
...
export default function SurveyPlanPanel({ projectId }: SurveyPlanPanelProps) {
  ...
  if (loading) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--theme-fg-secondary, #6B7280)' }}>
        <div style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>⏳</div>
        Generating field survey plan…
        <div style={{ fontSize: '0.8rem', marginTop: '0.5rem', color: 'var(--theme-fg-muted, #9CA3AF)' }}>
      ...
      <div style={{ width: 180, flexShrink: 0 }}>
        <div style={{ position: 'sticky', top: 0 }}>
          {sections.map(s => (
            ...
            >
              ...
              {s.key === 'research' && totalPre > 0 && (
                <span style={{ float: 'right', fontSize: '0.7rem', color: 'var(--theme-fg-secondary, #6B7280)' }}>
          ...
          <div style={{ marginTop: '1rem', borderTop: '1px solid #E5E7EB', paddingTop: '0.75rem' }}>
            <button
              onClick={() => void loadPlan()}
              style={{ display: 'block', width: '100%', padding: '0.4rem 0.75rem', borderRadius: 6, border: '1px solid #D1D5DB', background: '#fff', color: 'var(--theme-fg-secondary, #374151)', fontSize: '0.75rem', cursor: 'pointer' }}
            >
              ↻ Refresh Plan
            </button>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', maxHeight: 700 }}>
        ...
        {activeSection === 'summary' && (
          <div>
            ...
            <p style={{ fontSize: '1rem', lineHeight: 1.7, color: 'var(--theme-fg-primary, #111827)', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '0.75rem' }}>
                {plan.key_facts.map((f, i) => (
                  <div key={i} style={{ padding: '0.75rem', background: '#F9FAFB', borderRadius: 8, border: '1px solid #E5E7EB' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--theme-fg-secondary, #6B7280)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>{f.label}</div>
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--theme-fg-primary, #111827)' }}>{f.value || '—'}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeSection === 'research' && (
          <div>
            ...
            {plan.pre_field_research?.description && (
              <p style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #6B7280)', marginBottom: '1rem' }}>
                {plan.pre_field_research.description}
              </p>
            )}
            {completedPre > 0 && (
          ...
          <div>
            ...
            {(plan.equipment_checklist?.items || []).map((cat, i) => (
              <div key={i} style={{ marginBottom: '1.25rem' }}>
                <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  {cat.category}
                </div>
                <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                  {cat.items.map((item, j) => (
                    <li key={j} style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.25rem' }}>{item}</li>
                  ))}
                </ul>
              </div>
            ))}
                ...
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--theme-fg-primary, #111827)' }}>{step.title}</span>
                      {step.phase && <span style={{ fontSize: '0.75rem', color: 'var(--theme-fg-secondary, #6B7280)', background: '#F3F4F6', padding: '1px 8px', borderRadius: 10 }}>{step.phase}</span>}
                      {step.estimated_time && <span style={{ fontSize: '0.75rem', color: 'var(--theme-fg-secondary, #6B7280)' }}>⏱ {step.estimated_time}</span>}
                    </div>
                    <p style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)', margin: '0 0 0.5rem' }}>{step.plain_english}</p>
                    {step.technical_notes && (
                      <div style={{ fontSize: '0.8rem', color: 'var(--theme-fg-secondary, #6B7280)', background: '#F9FAFB', padding: '0.5rem 0.75rem', borderRadius: 4, borderLeft: '3px solid #D1D5DB' }}>
          ...
          <div>
            ...
            {(plan.monument_recovery?.monuments || []).map((mon, i) => (
              <div key={i} style={{ marginBottom: '1rem', padding: '0.875rem', background: '#FAFAFA', borderRadius: 8, border: '1px solid #E5E7EB' }}>
                <div style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--theme-fg-primary, #111827)', marginBottom: '0.5rem' }}>
                  {mon.location} — <span style={{ fontWeight: 400, color: 'var(--theme-fg-secondary, #6B7280)' }}>{mon.type}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem 1rem' }}>
                  <div>
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--theme-fg-muted, #9CA3AF)', textTransform: 'uppercase', marginBottom: '0.2rem' }}>How to Find</div>
                  ...
                  <div style={{ gridColumn: '1 / -1' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--theme-fg-secondary, #374151)' }}>{mon.not_found_action}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {activeSection === 'boundary' && (
          <div>
            ...
            {plan.boundary_reconstruction?.description && (
              <p style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #6B7280)', marginBottom: '0.75rem' }}>
                {plan.boundary_reconstruction.description}
              </p>
            )}
            {plan.boundary_reconstruction?.method && (
              <div style={{ marginBottom: '1rem', padding: '0.75rem', background: '#EFF6FF', borderRadius: 8, border: '1px solid #BFDBFE' }}>
                ...
                <p style={{ margin: '0.25rem 0 0', fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)' }}>
            ...
            {plan.boundary_reconstruction?.priority_evidence && plan.boundary_reconstruction.priority_evidence.length > 0 && (
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.5rem' }}>Evidence Priority (highest to lowest):</div>
                <ol style={{ margin: 0, paddingLeft: '1.5rem' }}>
                  {plan.boundary_reconstruction.priority_evidence.map((e, i) => (
                    <li key={i} style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.2rem' }}>{e}</li>
                  ))}
                </ol>
              </div>
            )}
              ...
              plan.discrepancies_to_investigate.map((d, i) => (
                <div key={i} style={{ marginBottom: '1rem', padding: '0.875rem', background: '#FAFAFA', borderRadius: 8, border: '1px solid #E5E7EB' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                    ...
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--theme-fg-primary, #111827)', flex: 1 }}>{d.description}</div>
                  </div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--theme-fg-secondary, #374151)', padding: '0.4rem 0.6rem', background: '#F3F4F6', borderRadius: 4 }}>
          ...
          <div>
            ...
            {(plan.office_to_field_sequence || []).map((day, i) => (
              <div key={i} style={{ marginBottom: '1rem' }}>
                ...
                <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                  {day.tasks.map((t, j) => (
                    <li key={j} style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.25rem' }}>{t}</li>
                  ))}
                </ul>
              </div>
            ))}
              ...
              <div style={{ marginTop: '1.5rem' }}>
                ...
                {plan.special_considerations.map((c, i) => (
                  <div key={i} style={{ marginBottom: '0.75rem', padding: '0.75rem', background: '#F9FAFB', borderRadius: 8, border: '1px solid #E5E7EB' }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--theme-fg-secondary, #374151)', textTransform: 'uppercase', marginBottom: '0.25rem' }}>{c.category}</div>
                    <div style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)' }}>{c.description}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeSection === 'nextsteps' && (
          <div>
            ...
            {plan.next_steps && plan.next_steps.length > 0 ? (
              <ol style={{ paddingLeft: '1.5rem', margin: 0 }}>
                {plan.next_steps.map((step, i) => (
                  <li key={i} style={{ fontSize: '0.875rem', color: 'var(--theme-fg-secondary, #374151)', marginBottom: '0.75rem', lineHeight: 1.5 }}>
                    {step}
                  </li>
                ))}
              </ol>
            ) : (
              <p style={{ color: 'var(--theme-fg-secondary, #6B7280)' }}>No additional steps at this time.</p>
            )}
            <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#EFF6FF', borderRadius: 8, border: '1px solid #BFDBFE' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--theme-fg-secondary, #374151)', margin: 0, lineHeight: 1.6 }}>
                Set the following environment variables to activate the complete pipeline:
              </p>
              <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.5rem', fontSize: '0.85rem', color: 'var(--theme-fg-secondary, #374151)' }}>
            ...
            <div style={{ marginTop: '0.75rem', fontSize: '0.75rem', color: 'var(--theme-fg-muted, #9CA3AF)' }}>
