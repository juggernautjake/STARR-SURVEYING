...
// and /admin/finances have the SAME middleware entry — admin, developer, tech_support — and the
// same three roles on their registry rows. The door does not move and no list moves with it.
//
// The boundary is real too, checked rather than assumed: /api/admin/reports/* resolves the
// caller's org membership and refuses anyone whose role on that org is not admin. A 403, not a
// row filter and not a 401-only pass — which is more than three other endpoints in this plan
// turned out to have.
'use client';
