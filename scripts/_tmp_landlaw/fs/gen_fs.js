...
for (let N = 1; N <= 10; N++) {
  ...
  (data.problem_templates || []).forEach((t, i) => {
    ...
    W(`INSERT INTO problem_templates (id, name, description, category, subcategory, question_type, difficulty, question_template, answer_formula, answer_format, parameters, computed_vars, solution_steps_template, options_generator, explanation_template, module_id, exam_category, tags, is_active, created_by) VALUES (`);
    W(`  ${S(tid)}, ${S(t.name)}, ${S(t.description||t.name)}, ${S(t.category||'FS — Computations')}, ${S(t.subcategory||null)}, ${S(t.question_type||'numeric_input')}, ${S(t.difficulty||'medium')}, ${S(t.question_template)}, ${S(t.answer_formula)}, ${Jtext(af)}, ${J(t.parameters||[])}, ${J(t.computed_vars||[])}, ${J(t.solution_steps_template||[])}, ${J(t.options_generator||{method:'none'})}, ${S(t.explanation_template||'')}, ${S(mid)}, 'FS', ${arr(['fs-buildout',`fs-m${N}`,'fs-practice'])}, true, ${S('fs:m'+N)});`);
