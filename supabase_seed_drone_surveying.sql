-- ============================================================================
-- STARR Surveying — Drone Surveying Module Seed Data
-- ============================================================================
-- Standalone module: UAS (Unmanned Aircraft Systems) for Land Surveying
--   12 instructional lessons + 1 final exam = 13 total
--   5 quiz questions per lesson = 60 quiz questions
...
DELETE FROM learning_lessons WHERE module_id = 'da5e0001-0000-0000-0000-000000000001';
...
-- ============================================================================
-- SECTION 3: QUESTION BANK — LESSON QUIZZES (5 per lesson × 12 = 60)
-- ============================================================================

DELETE FROM question_bank WHERE tags @> ARRAY['drone-surveying'];
...
('A camera has a pixel size of 4.4 micrometers and a focal length of 8.8 mm. Flying at 100 m AGL, the ground sample distance (GSD) in centimeters is approximately:', 'numeric_input',
 '[]'::jsonb,
 '5.0',
 'GSD = (pixel_size × altitude) / focal_length = (0.0044 mm × 100,000 mm) / 8.8 mm = 440 / 8.8 = 50 mm = 5.0 cm. Each pixel in the image represents approximately 5 cm on the ground.',
...
('Flying at a higher altitude increases the ground sample distance, resulting in lower image resolution.', 'true_false',
 '["True","False"]'::jsonb,
 'True',
 'GSD is directly proportional to altitude: GSD = (pixel_size × altitude) / focal_length. Flying higher means each pixel covers a larger area on the ground, reducing the level of detail captured. Lower altitude = smaller GSD = higher resolution.',
...
('LiDAR measures distance by:', 'multiple_choice',
 ...
 'LiDAR (Light Detection and Ranging) emits a laser pulse and measures the time for it to travel to a surface and return. Distance = (speed of light × round-trip time) / 2. The laser scanner rapidly fires thousands of pulses per second to build a dense 3D point cloud.',
...
('Ground sample distance (GSD) is affected by all of the following EXCEPT:', 'multiple_choice',
 ...
 'GSD = (pixel_size × altitude) / focal_length. It depends on camera pixel size, flight altitude, and focal length. GCPs affect georeferencing accuracy but have no effect on the pixel resolution (GSD) of the imagery.',
...
('Stockpile volume is calculated by:', 'multiple_choice',
 ...
 'Volume is the integral of the height difference between the stockpile surface and a base surface. The base can be a pre-stockpile survey, a flat plane, or a surface triangulated from the stockpile toe. Simply using max height × area would grossly overestimate volume.',
...
('RMSE is calculated as:', 'multiple_choice',
 ...
 'RMSE = sqrt(Σ(error²) / n), where error is the difference between computed and known coordinates at each checkpoint, and n is the number of checkpoints. It gives more weight to large errors than a simple average.',
...
('A camera with a pixel size of 5 micrometers and a focal length of 10 mm is flown at 120 m AGL. The resulting GSD in centimeters is:', 'numeric_input',
 '[]'::jsonb,
 '6.0',
 'GSD = (pixel_size × altitude) / focal_length = (0.005 mm × 120,000 mm) / 10 mm = 600 / 10 = 60 mm = 6.0 cm.',
