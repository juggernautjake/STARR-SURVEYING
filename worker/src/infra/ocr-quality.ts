// worker/src/infra/ocr-quality.ts — an unreadable page must say so, on BOTH paths (plan R18).
//
// This lives in the worker and is re-exported by `lib/research/ocr-quality.ts` so there is exactly
// ONE definition of "readable". The two extraction paths had already drifted apart on a related
// constant — 500 chars in the app, 800 in the worker, with a comment at the call site explaining the
// difference — which is how a rule ends up enforced on one path and not the other.
//
// ── THE DEFECT ──────────────────────────────────────────────────────────────────────────────────
//
