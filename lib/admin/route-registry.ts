...
export const ADMIN_ROUTES: AdminRoute[] = [
  ...
  // The brand system: colours, type, every approved mark, and the building blocks made from them.
  // Office rather than a workspace of its own — it is a reference somebody visits occasionally,
  // which is exactly what 'Documents & records' already holds.
  { href: '/admin/branding',        label: 'Brand & style guide', workspace: 'office', section: 'Documents & records', iconName: 'Palette', description: 'The approved logos, colours and typefaces, with the ink rule and every measured pairing. What to hand a printer.', roles: ['admin', 'developer', 'tech_support', 'teacher', 'employee'], internalOnly: true, keywords: ['brand', 'branding', 'logo', 'logos', 'colour', 'color', 'palette', 'hex', 'font', 'fonts', 'typeface', 'typography', 'style guide', 'identity', 'mark', 'badge', 'roundel', 'swatch', 'pantone', 'cmyk', 'merch', 'apparel', 'shirt', 'hat', 'cap', 'embroidery', 'print', 'printer', 'signage', 'vehicle wrap', 'heritage', 'star'] },
  ...
  { href: '/admin/calls', label: 'Calls', workspace: 'work', iconName: 'Phone', description: 'Every call to the business line: who called, the recording, the transcript, and what the AI made of it. Turn a call into a lead or a project.', roles: ['admin'], internalOnly: true, keywords: ['phone', 'call', 'calls', 'voicemail', 'recording', 'transcript', 'receptionist'] },
  ...
  // overheard on a call (`observed`, and it must ask). Palette-only and reached from the Calls
  // header: it is a settings surface for one page, not a second phone row on the rail.
  { href: '/admin/calls/registry', label: 'Caller Registry', workspace: 'work', iconName: 'Contact', description: 'Who each phone number belongs to, and how sure we are. Confirm a name so the receptionist greets that caller by it, or tell it never to put a name to a shared number.', roles: ['admin'], internalOnly: true, showInRail: false, keywords: ['caller', 'caller id', 'registry', 'contact', 'contacts', 'phone book', 'phonebook', 'number', 'who is calling', 'name', 'receptionist', 'memory', 'verified', 'observed', 'never assume', 'call log'] },
  // In the rail since 2026-09-16: it was palette-only, and the owner could not find the page he is
  // meant to be honing the agent on. It sits next to Calls, which is where its results land.
  { href: '/admin/dev/receptionist', label: 'Receptionist Test', workspace: 'work', iconName: 'FlaskConical', description: 'Talk to the phone receptionist from the browser, your own phone, or by text — same voice and timing as the business line, flagged as a test.', roles: ['admin', 'developer'], internalOnly: true, keywords: ['receptionist', 'test', 'ellie', 'phone', 'voice', 'relay', 'developer', 'dev', 'talk', 'agent', 'elevenlabs'] },
