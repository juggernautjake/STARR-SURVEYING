...
export const PF2_FEATS_CLASS_GAPS: string[] = [
  'CLASSES WITH NO ENTRIES AT ALL: Oracle, Witch, Magus and Summoner. All four are catalogued in content.ts as playable. Oracle and Witch key off subsystems (curses/mysteries, patrons/hexes) whose Remaster names and feat levels I could not confirm. Magus and Summoner were catalogued on 2026-08-02 from their class chassis alone — key attribute, HP, skills, proficiencies, the level-1 choice — which is enough to BUILD one and not enough to author a feat list; their feats key off Hybrid Studies and eidolon types respectively. A Magus is therefore fully playable and takes its class-feat slots as custom entries, which is the escape hatch working as designed rather than a hole.',
