...
export const PREFERENCE_HELP: PreferenceHelp[] = [
  ...
  { key: 'proficiencyWithoutLevel', name: 'Proficiency without level (PF2)', explains: 'Pathfinder 2e only, and an official GM Core variant rather than a house rule. Normally PF2 adds your character level to every proficiency-based number — checks, saves, AC, class and spell DCs, Strikes. With this on it does not, and being untrained costs −2 instead of nothing. The effect is a flatter game: a low-level monster is still a threat at 12th level, and a high-level DC is still reachable at 3rd. It changes every number on the sheet at once, so the sheet shows a "variant rules in force" note while it is on.' },
  { key: 'freeArchetype', name: 'Free archetype (PF2)', explains: 'Pathfinder 2e only (GM Core variant). You gain an extra class feat at 2nd level and every even level after, which may be spent only on archetype feats. Your normal class feats are untouched — this is granted on top of them, so a character can pursue an archetype without giving up their own class’s feats.' },
  { key: 'startingHeroPoints', name: 'Starting Hero Points (PF2)', explains: 'Pathfinder 2e only. How many Hero Points a character begins a session with; the rules say 1. Spend one to reroll a check, or all three to avoid death. This one applies when a character is BUILT rather than being recalculated live, because Hero Points are spent down during play — changing the setting does not retroactively refill an existing character.' },
