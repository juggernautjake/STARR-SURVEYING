...
function MileageTrackerWidget({ content }: WidgetProps<MileageTrackerContent>) {
  ...
  if (status === 'empty' || !summary) return <WidgetEmpty icon="🚗" title="No mileage" description="Trips log here as you drive." />;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <span style={{ fontSize: 'var(--hub-font-2xl, 1.5rem)', fontWeight: 700, color: 'var(--theme-accent)' }}>{summary.miles.toFixed(1)} mi</span>
      <span style={{ fontSize: 'var(--hub-font-xs, 0.75rem)', color: 'var(--theme-fg-secondary)' }}>{summary.trips} trips · ${summary.reimbursable_amount.toFixed(2)} reimbursable</span>
    </div>
  );
}
