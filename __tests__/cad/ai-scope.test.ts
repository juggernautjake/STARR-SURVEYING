...
describe('a pinned scope outlives the features in it', () => {
  ...
  it('reports the shortfall instead of silently shrinking', () => {
    ...
    expect(scopeStaleCount(DOC, { kind: 'IDS', ids: ['a', 'ghost', 'gone'] })).toBe(2);
    expect(scopeStaleCount(DOC, { kind: 'IDS', ids: ['a', 'd'] })).toBe(0);
  });
...
describe('resolving which ids a turn acts on', () => {
  it('the pin wins over the live selection', () => {
    ...
    expect(resolveScopeIds({ kind: 'IDS', ids: ['a', 'b'] }, ['d', 'e', 'f'])).toEqual(['a', 'b']);
  });
  ...
  it('an empty pin is treated as a pin, not as absent', () => {
    ...
    expect(resolveScopeIds({ kind: 'IDS', ids: [] }, ['d'])).toEqual([]);
  });
});
...
describe('the store', () => {
  ...
  it('refuses to pin an empty selection', () => {
    ...
    // C33 wrapped the ids in a discriminated `ScopeRef`, because a layer scope is a different kind
    // of thing that resolves live. The refusal itself is unchanged.
    expect(src).toMatch(/ids\.length > 0 \? \{ kind: 'IDS', ids: \[\.\.\.ids\] \} : null/);
  });
...
describe('the chip', () => {
  ...
  it('shows the resolved scope, so pinned and live read the same way', () => {
    // C33 added `doc`, which a LAYER scope needs to resolve against.
    expect(src).toMatch(/resolveScopeIds\(pinnedScope, live, doc\)/);
