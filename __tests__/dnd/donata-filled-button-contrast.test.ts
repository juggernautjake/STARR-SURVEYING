...
//     .btn.danger { color:#fff; background: linear-gradient(160deg, #f0577a, var(--danger)) }
...
describe('the rules are still shaped the way this measurement assumes', () => {
  it('donata paints these buttons as gradients with a white label', () => {
    ...
    expect(CSS).toContain('.dnd-sheet.skin-donata .btn.danger { color: #fff; background: linear-gradient(160deg, #f0577a, var(--danger))');
...
describe('the LIGHT stop is where all three fail — the open item, with numbers', () => {
...
describe('two remedies, both measured, both keeping the skin', () => {
  ...
  it('B does NOT work on the danger stop — so that one wants option A', () => {
    ...
    expect(contrastRatio(DONATA_INK, LIGHT_STOPS['btn.danger'])!).toBeLessThan(AA);
  });
