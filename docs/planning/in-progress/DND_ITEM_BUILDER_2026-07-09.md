...
- [ ] **Slice 4 — The builder UI.** Replace Inventory's minimal add-form with a real **Item Builder**: pick a `kind`, fill title/description/qty, then kind-specific fields (weapon dice+type+bonus-dice rows; armor category+AC; consumable effect; wondrous effects); per-item **image upload** (`kind:'item'`). Edit existing items through the same builder. Robust validation. Readable on all skins. **DM item-granting:** because the DM can edit every character in the campaign, the builder on a player's sheet already lets the DM create + place items in that inventory — add a clear, simple **"Grant to a character"** affordance (a character picker) so the DM can build an item once and drop it into any player's inventory without leaving the builder.
