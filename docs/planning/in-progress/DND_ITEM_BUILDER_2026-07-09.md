...
- [ ] **Slice 4b — DM grant to a character.** Because the DM can edit every character in the campaign, the builder on a player's sheet already lets the DM create + place items in that inventory. Add a simple **"Grant to a character"** picker so the DM can build an item once and drop it into any player's inventory (PATCH the target character's `data`) without leaving the builder.
