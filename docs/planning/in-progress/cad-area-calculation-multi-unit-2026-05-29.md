...
#### Slice 230 — Drag-to-move the area label
- **Scope:** Hit-test the rendered Pixi Text objects under the cursor so the surveyor can grab the label + drag it to a new position. On pointer-up call `useAnnotationStore.updateAnnotation(id, { position: { x: worldX, y: worldY } })` to commit. Drag should follow the same `labelDragRef`-style pipeline the existing per-feature `textLabels` use. (Captured but not yet shipped.)
