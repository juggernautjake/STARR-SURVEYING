...
#### Slice 228 — Surface multi-unit area in PropertyPanel for every closed shape
- **Scope:** Replace the POLYGON-only `A:` readout in
  `PropertyPanel.tsx` with a generic area row driven by
  `computeFeatureArea(feature)` whenever the selected feature
  reports a non-zero area. Show the value in the surveyor's
  display-pref unit AND in sq-ft + acres as a secondary line so
  the conversion is always one glance away.
- **Done when:** Selecting any closed shape shows its area in the
  property panel.
