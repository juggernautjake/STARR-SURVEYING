...
| **B** | **Tool-registry expansion** — register slice A solvers in `lib/cad/ai/tool-registry.ts` as new AI tools: `calcFourthRectangleCorner`, `calcPointFromTwoBearings`, `calcPointParallelToLine`, `inverseTwoPoints`, `closureReport`, `bowditchAdjust`. Update system prompt so the AI knows to call solvers instead of guessing. | ⏳ |
