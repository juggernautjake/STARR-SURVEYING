...
- [x] **Confirmed genuine stubs — banner correctly KEPT (accurate):** `leads`, `schedule`, `notes`, `my-files` all use `const [x] = useState([])` with no setter/fetch/persistence; `settings` has an explicit in-code TODO "Connect form inputs to settings API." Per the user's rule the banner stays until these are built. Tracked as a future build-out slice (each needs a table + API + wiring — larger than the current priority).
...
- [ ] **Follow-up:** add a web control to *assign/reassign* a receipt's `job_id` from the receipts approval page (the link is currently set at mobile submission). Verify `PATCH /api/admin/receipts/[id]` accepts `job_id` first.
