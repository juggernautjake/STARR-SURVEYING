...
- [x] User: "as the user increases/decreases line weight or font size … the affected features render the change dynamically." In `PropertyPanel`, single-feature **Color / Line Weight / Opacity** edits only updated local state and committed to the canvas on **blur** — no live feedback (bulk edits already applied live).
  ...
  - **Verified:** `tsc` + `eslint` clean. **Harness E2E was attempted but not retained:** reliably creating + selecting a feature from Playwright proved infeasible (tool keys are chord-prefixed, WebGL canvas pointer events and command-bar coordinate entry didn't register deterministically across ~8 runs). The change is code-verified — it routes through the exact `updateFeature` path the bulk editor already uses live, just on `onChange` instead of `onBlur`; prior visual audits confirmed the canvas repaints on store updates.
- [ ] **Next:** apply the same live-on-change pattern to font-size (and other text-style) edits wherever labels/text are styled (e.g. `FeaturePropertiesDialog` / label-style controls).
