...
| 10 | Reports: `/admin/invoicing/reports` (target vs actual per category + revenue per period + outstanding invoices) | UI | Pure-aggregator helper underneath so the data layer is testable independent of the table styling. |
| 11 | Category editor: `/admin/invoicing/categories` (add / archive / set percentages) | UI | Sum-of-percentages must equal 100% (validate at save). |
