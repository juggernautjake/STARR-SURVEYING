...
| R4b | **4 left, and the count was wrong before.** The inventory read 0 on 2026-08-04; measured properly it was 6. Six Bell analyzers accumulated a usage summary that was displayed and never persisted — `recordAiUsage` was defined in `ai-cost-helpers.ts` and called from nowhere in the repository. They passed the ratchet because its predicate is a text search and they *import* that module. Deed + plat now record (projectId threaded from the orchestrator, one accurate record per run since every call goes through `modelFor('read_scan')`); the other four are listed in `UNMIGRATED` with per-file reasons | The ratchet now sweeps `.messages.create(` as well as `new Anthropic(`, requires a recorder to be **called** rather than merely imported, and asserts the orchestrator still threads projectId. The last of those was added because its negative control did **not** fire — `projectId` is optional, so cutting the thread compiled and passed |
