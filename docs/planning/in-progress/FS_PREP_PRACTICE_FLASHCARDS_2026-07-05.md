...
| **P7** | Author FS flashcards (~15–25/module via `m<N>.json` + `gen_fs.js`), seed + apply | **IN PROGRESS** — Module 1 authored: 20 cards in `fs/m1.json` `flashcards[]` (derived from its vetted key_topics/formulas). New reproducible generator `gen_fs_flashcards.js` emits idempotent `seeds/402_fs_prep_flashcards.sql` (tag `fs-flashcards`/`fs-m<n>`, category `fs:m<n>`, deterministic UUIDs, DELETE-then-INSERT). **Not yet applied to live** — awaiting go-ahead before writing to prod Supabase. **Authoring COMPLETE — all 10 modules, 190 cards** in `seeds/402` (m1-2 & m6-8 20 each, m3 19, m4 16, m5 19, m9-10 18 each; covers stats, leveling, distance/angle, traverse/COGO, areas/volumes/curves, GNSS/geodesy, boundary law/PLSS, photogrammetry/GIS, calculator mastery, and comprehensive review). Only remaining P7 step: **apply seed 402 to live Supabase + verify** — held for explicit go-ahead before writing to prod (per node-pg + SUPABASE_DB_URL workflow). Once applied, `discover_module_cards` (P5) surfaces the cards automatically. |
