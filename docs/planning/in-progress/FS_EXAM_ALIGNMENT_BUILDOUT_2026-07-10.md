...
| **S17** | Mapping/Photogrammetry templates. | **DONE** (9de7d9bc) — 5 templates (40/40), 3 attaching figures: Q6 sewer cut + profile fig, Q7 slope-stake offset + cross-section fig, Q9 highest contour + contour fig, Q10 & Q14 photo scale. Concepts: Q8 FEMA cert, Q11 GIS topology, Q12 NSSDA 1.9600 (+1.7308), Q15 LiDAR + raster/vector, relief displacement. Q13 drag-label shipped S3/S8. 0 MC mismatches. Seed 433, applied to live DB. |
| **S18** | Surveying Processes/Principles questions. | **DONE** (cfbe1698) — 10 concept questions: Q5 survey-type (topographic), Q34 error ellipses, Q50 redundant observation + siblings (survey-type purposes, plane vs geodetic, semi-major axis, least-squares minimization, redundancy). Q1 multi_select shipped S1. 0 MC mismatches. Seed 434. **Phase D complete — all 50 exam questions covered.** |
| **S26** | Figure↔question consistency verification. | **DONE** (d24979a0) — `npm run verify:fs-figures` generates 8 instances per figure template and asserts figure numbers == question numbers + internal derivations (curve T/LC/E/M, plat remainder=M−4w, contour interval): **ALL PASS (56/56)**. Rendered every figure type to PNG via Playwright and visually confirmed correctness; fixed 2 cosmetic label overlaps (tower A/B, curve I/PI). Data is consistent every time. |
