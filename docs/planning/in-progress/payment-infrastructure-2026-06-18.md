...
| **P12** ✅ | **Approval flow.** Designated payout-admin (today: daddy / Hank) reviews a draft batch and clicks "Approve". Status → `approved`. Approval signature stored (admin email + timestamp + IP). Unapproved batches can't fan out. _Shipped — `lib/payouts/approval.ts` pure helpers (`isInPayoutAdminAllowlist` reads env `PAYOUT_ADMIN_EMAILS` comma-separated list with trim+lowercase, `canApprovePayoutBatch` allowlist-then-role fallback so admins keep approval rights in dev until the env var is wired, `extractRequestIp` honors `x-forwarded-for` first entry → `x-real-ip` fallback). `GET /api/admin/payouts/runs/[id]` returns header + items. `POST /api/admin/payouts/runs/[id]/approve` gated by `canApprovePayoutBatch`, refuses non-draft batches with 409, **blocks self-approval** (creator can't approve own batch unless `PAYOUT_ADMIN_SELF_APPROVE` env override is set — separation of duties), stamps `approved_by` + `approved_at` + `approval_ip`. `POST /api/admin/payouts/runs/[id]/void` only on draft / approved (dispatched / completed 409 since they need a refund flow), stitches the void reason into `notes` for the audit trail, stamps `voided_by` + `voided_at`. `app/admin/payouts/runs/[id]/page.tsx` detail surface — status chip, approval-signature callout box (or void callout), full line-item table, approve button (visible only on draft), void prompt with optional reason. History list rows on `/admin/payouts/runs` now link to the detail page. Source-locked by `__tests__/admin/payment-payout-approval.test.ts` (25 green; P11 still green — 58 total)._ |
| **P13** ✅ | **Outbound dispatch — per-method.** Each employee's preferred method dictates the dispatch path:<br>- **Stripe Connect / Treasury** (when wired) — instant payouts to a verified bank.<br>- **Venmo / CashApp / Zelle** — generate a deep-link list the payout admin works through one tap at a time, marking each as sent.<br>- **Cash** — generate a single "cash to hand out" list; admin physically distributes + marks each as paid.<br>- **Bank ACH (manual NACHA)** — generate a NACHA-compliant batch file the office uploads to PNC's portal. _Shipped — `lib/payouts/dispatch.ts` pure helpers (`groupItemsByMethod` buckets including `unassigned`, `buildPayoutNote` single source of truth for the memo, `buildPayoutDeepLink` per platform: Venmo deep-link / Cash App URL / Zelle mailto fallback / strips leading @ + $, returns null for cash/ach + missing-handle rows, `batchStatusFromItems` lifecycle roll-forward — all-paid → completed, mixed sent/paid → dispatched, all-pending → approved; voided/draft pass through unchanged). New routes: `POST /api/admin/payouts/runs/[id]/items/[itemId]/mark` (status state machine: pending / sent / paid / failed; recomputes parent batch status after every flip + stamps `dispatched_at` / `completed_at`); `GET /api/admin/payouts/runs/[id]/ach-csv` (text/csv attachment with the simple `email,name,account,amount,memo` format PNC's Bill Pay accepts — full NACHA encoding deferred until the bank confirms their preferred upload format). New page `/admin/payouts/runs/[id]/dispatch`: per-method sections, deep-link "Open" button per row, optional external-ref input, Mark sent / Mark paid / Mark failed / Reopen buttons; ACH section shows "Download CSV for PNC upload". Detail page now shows "Open dispatch →" link for approved + dispatched batches. **Stripe Connect / Treasury integration intentionally deferred** — requires bank account + Stripe Connect onboarding which the user already said is the long-pole; current architecture treats `stripe` as a future method that can drop in without schema changes. Source-locked by `__tests__/admin/payment-payout-dispatch.test.ts` (29 green; P11 + P12 still green — 87 total)._ |
| **P14** ✅ | **Per-payout audit trail.** Every payout row tracks `attempted_at`, `cleared_at`, `method`, `external_ref` (Venmo transaction id, NACHA file id, etc.), `notes`. Visible on the employee's profile page. _Shipped — `seeds/326_payout_attempted_at.sql` adds the `attempted_at TIMESTAMPTZ` column + a `(user_email, attempted_at DESC)` index for the per-employee lookup. The dispatch mark route now stamps `attempted_at` on the FIRST non-pending transition (sent / paid / failed); subsequent flips don't re-stamp; reopen-to-pending clears it so a retry stamps fresh. New `GET /api/admin/employees/[email]/payouts` returns the audit trail (admin OR self-view, 100 newest-first, joins parent batch for label + range). New `EmployeePayoutHistory` client component renders one card per payout: batch label, status chip, amount, method, attempted/cleared timestamps, external reference (monospace), failure_reason + notes when present, empty state. Wired into the existing `/admin/employees/[email]/page.tsx` profile. Source-locked by `__tests__/admin/payment-payout-audit-trail.test.ts` (19 green; P13 still green)._ |
| **P15** ✅ | **One-time bonuses + reimbursements.** Outside the weekly batch, the office can fire a single ad-hoc payout to an employee (bonus check, expense reimbursement). Same approval flow, same audit trail. _Shipped — `app/admin/payouts/ad-hoc/page.tsx` focused single-employee form: kind picker (bonus / reimbursement / other), employee email + name, amount $, optional method + handle, required reason memo (lands in `notes` for the audit trail). Bonus + other map to `bonuses_cents`; reimbursement maps to `reimbursements_cents` so the per-employee totals stay categorized. POSTs to the existing `/api/admin/payouts/runs` with `kind: 'ad_hoc'` + a single item; the route already short-circuits the week-start/end requirement for ad-hoc batches (P11 schema). Auto-labels the batch with the kind + email + amount for easy scanning ("Bonus · mary@starr.com · $500.00"). Redirects to the detail page after creation so the office can pass it to the payout admin for approval (same P12 flow). New "One-off payout" link on `/admin/payouts/runs` next to the "New batch" button. The downstream approval / dispatch / audit trail flows are unchanged — ad-hoc batches flow through P12 → P13 → P14 exactly like weekly ones. Source-locked by `__tests__/admin/payment-payout-ad-hoc.test.ts` (13 green)._ |
| **P16** ✅ | **Tax reporting prep.** Annual + quarterly export — list every payout per employee in a 1099-friendly + W-2-friendly format. Stripe Tax handles the Stripe Connect path automatically when wired; manual exports cover Venmo / cash / NACHA. _Shipped — `lib/payouts/tax-report.ts` pure helpers (`aggregateForTaxReport` folds paid items per employee + per method into the canonical row shape, sorts descending by total, lowercase email, normalizes unknown methods into `other`, clamps negative cents to zero, tracks first/last paid_at + payment count; `totalsAcrossRows` grand totals; `buildTaxCsv` emits 13-column header `user_email, user_name, payment_count, first_paid_at, last_paid_at, total_usd, venmo/cashapp/zelle/ach/cash/stripe/other_usd` with CSV-safe quoting; `describeRange` labels Jan 1 → Dec 31 as "Tax year YYYY", calendar quarters as "Q1 2026" etc, otherwise "from → to"). `GET /api/admin/payouts/tax-report?from=YYYY-MM-DD&to=YYYY-MM-DD[&format=csv]` filters `payout_batch_items.status='paid'` on `paid_at` inclusive; admin-gated; supports both JSON (for the on-page table) and text/csv download. New page `/admin/payouts/tax-report`: year + 4-quarter quick-pin buttons, custom from/to inputs, per-employee table with name + method-chip breakdown, grand total, empty state. "Tax report" link added to the runs page header. Doesn't classify employees as 1099 vs W-2 — that's the tax preparer's call; the office annotates downstream. Source-locked by `__tests__/admin/payment-payout-tax-report.test.ts` (21 green)._ |

### Security + compliance (P17–P19)

| Slice | What ships |
|---|---|
| **P17** ✅ | **Secrets handling.** Bank account numbers + ACH routing numbers encrypted at rest via pgcrypto (`pgp_sym_encrypt` keyed by `PAYMENT_ENCRYPTION_KEY` env var). Encryption key rotation script. No card numbers ever touch the database — Stripe Elements tokenises in the browser; we only store the Stripe payment_intent id. Audit-log every read of the encrypted columns. _Shipped — `seeds/327_payment_secret_audit.sql` adds `payment_secret_reads` audit table (reader email + IP, target table + id, subject email, field name, reason, succeeded + error_message, three lookup indexes, service-role RLS only — no employee-self-read). `lib/payments/secrets.ts` pure helpers (`assertEncryptionKeyConfigured` refuses placeholders + sub-16-char keys, `maskAchAccount` returns `•••1234` for UI display, `ACH_READ_REASONS` enum (`payroll_dispatch` / `employee_self_view` / `admin_review` / `audit` / `rotation`), `buildAuditEntry` shape function, `scrubAchFromMessage` defense-in-depth regex that masks any 8+ digit run in logs). `scripts/rotate-payment-encryption-key.mjs` (paginated rotation: decrypt with old key → encrypt with new key → round-trip verify with new key before UPDATE → stamps `payment_secret_reads` audit row per touched row with reason=`rotation`; refuses to run with weak / matching keys; supports `--dry-run`; exits non-zero on any error). New `docs/security/payments-pci-scope.md` documents PCI scope per method (Stripe SAQ A, everything else out of scope), encryption posture, audit log, rotation runbook, RLS posture table, `PAYMENTS_LIVE` live-money gate, deferred items (Stripe Connect/Treasury, full NACHA, AML/OFAC). The seed 324 pgcrypto helpers `encrypt_ach_secret` / `decrypt_ach_secret` from P2 are what the rotation script + future app-layer reads call. Source-locked by `__tests__/admin/payment-secrets-rotation.test.ts` (26 green)._ |
| **P18** ✅ | **RLS audit.** Every payment + payout + invoice table gates SELECT/UPDATE to either service_role (admin paths) or row-owner-by-email (employee viewing own payout history). Customer-facing `/pay` reads through a service-role API path with an invoice-number-required gate (no enumeration). _Shipped two-layer audit. (1) Static: `lib/payments/rls-allowlist.ts` publishes the canonical list of all 9 payment-domain tables across seeds 323/324/325/327 + their expected employee-self-read posture + rationales; `expectedPolicyNamesForTable` helper resolves the canonical policy names (tolerating the pre-convention name `employee_self_read_payment_methods` that seed 324 ships); the vitest spec walks every (table, seed) pair + asserts the seed enables RLS + ships the `service_role_full_access_*` policy + (where applicable) an `employee_self_read_*` policy gated by `auth.jwt() ->> 'email'`. Customer-facing tables (`invoices`, `payments`, `payment_intents`, `payment_attempts`, `payment_receipts`) are explicitly asserted to NOT have an employee-self-read policy — they're route-mediated by invoice number / public_slug to prevent enumeration. (2) Runtime: `scripts/audit-payment-rls.sql` paste-into-Supabase query that joins `pg_class.relrowsecurity` + `pg_policies` against the same allowlist and emits a single `pass` boolean column so the office can `WHERE pass = FALSE` to surface drift after manual schema edits. `docs/security/payments-pci-scope.md` updated with the runbook link. Source-locked by `__tests__/admin/payment-rls-audit.test.ts` (32 green; all earlier payment specs still green — 101 across the 4 sanity-checked files)._ |
| **P19** ✅ | **PCI scope review.** Document the boundary: Stripe = SAQ A scope, all other methods (Venmo / CashApp / Zelle / cash / check) = out-of-scope for PCI. Note explicitly in the docs/security file. _Shipped — `docs/security/payments-pci-scope.md` extended with a "What we store vs. what Stripe stores" boundary table (full card / CVV / expiry / last-4 / cardholder name all "**never**" in our DB; we hold only the opaque Stripe `payment_intent.id` + the masked last-4 of the charge id for receipt display). New dedicated `docs/security/saq-a-eligibility-checklist.md` — one-page audit the office runs whenever the `/pay` route or the Stripe webhook changes. Sections: outsourced-cardholder-data conditions (Stripe Elements rendering, no raw card data on server, no raw card data in DB), hosted-by-validated-third-party conditions (iframe loaded from js.stripe.com, webhook signature-verified, secrets in env vars), webhook integrity (replay protection via `processed_webhook_events` + `ON CONFLICT DO NOTHING`, fail-closed on unverified events), non-Stripe paths each annotated with the file that enforces the out-of-scope posture, operational requirements (quarterly ASV scan, annual SAQ A attestation, incident-response runbook flagged for follow-up doc), explicit "what would push us OUT of SAQ A" list (adding a non-Stripe card capture flow, proxying the iframe, storing card last-4 / expiry, accepting card numbers through phone/email). Source-locked by `__tests__/admin/payment-pci-scope-review.test.ts` (13 green; P17 + P18 still green — 58 total). **Incident response runbook deferred** to its own follow-up doc as flagged — out of scope for the PCI review itself._ |

### Polish + QA (P20–P22)

| Slice | What ships |
|---|---|
| **P20** ✅ | **Customer portal styling pass.** Three styling passes per the user spec — verify mobile (one-thumb tap targets), tablet, desktop. Brand-consistent header. Status-pill colors for paid/pending/overdue. Loading skeletons. Error states for invalid invoice numbers. Accessibility: WCAG AA contrast, keyboard nav, screen-reader labels. _Shipped — `app/pay/PayHeader.tsx` sticky brand bar (red STARR mark + Surveying · Payments tail + tel: CTA), `role="banner"` landmark, descriptive `aria-label`s on brand + call links, 44×44 min tap target on the phone CTA (WCAG AA), brand tail + call text collapse on phones ≤480px. `app/pay/PaySkeleton.tsx` shimmer skeleton (`role=status`, `aria-busy=true`, `aria-live=polite`, visually-hidden text label for screen readers, `prefers-reduced-motion` suspends the shimmer animation). Pay.css gains `.visually-hidden` helper + universal `.pay-shell :focus-visible` 3px navy ring (3:1 contrast vs white). `/pay` landing + `/pay/[invoice]` detail both mount `<PayHeader />` on loading + error + success branches. Loading branch on the detail page now renders `<PaySkeleton />` instead of plain text. Method picker cards stamped with `aria-label="Pay $X with <method>"` so screen readers announce the action clearly; the glyph stays `aria-hidden`. Error branch uses `role=alert`. Status-pill colors from P4 already verify AA contrast (success #1f6d3c on #e2f5e8, warn #8a6300 on #fff4d6, danger #8a0e13 on #fdecec, info #1D3095 on #e6e9f6 — all ≥ 4.5:1). Source-locked by `__tests__/admin/payment-portal-styling.test.ts` (16 green; portal landing / deeplink / pledge / return-portal specs still green — 96 total)._ |
| **P21** ✅ | **Admin portal styling pass.** Three passes on the office surfaces — payment inbox, payout batch builder, audit views. Same polish bar. _Shipped — `app/admin/payments-admin.css` shared utility stylesheet scoped by `data-payments-admin` attribute on the root `<main>` of each admin page. Provides: (1) universal `:focus-visible` 3px navy ring matching the customer portal so muscle memory carries; (2) `.visually-hidden` sr-only helper; (3) `@media (prefers-reduced-motion: reduce)` blanket suspension of animations + transitions. All seven admin payment pages now import the stylesheet + stamp the data-attr: `/admin/invoices/new`, `/admin/payments/inbox`, `/admin/payouts/runs`, `/admin/payouts/runs/[id]`, `/admin/payouts/runs/[id]/dispatch`, `/admin/payouts/ad-hoc`, `/admin/payouts/tax-report`. Every visible error `<p>` now stamps `role="alert"`; the payments inbox loading state now publishes `role=status` + `aria-busy=true` + `aria-live=polite`; existing × remove buttons retain their `aria-label` from earlier slices. **Plus build-fix pass** — fixed every `react/no-unescaped-entities` lint error blocking the Vercel build (replaced literal `'` with `&rsquo;` and `"` with `&ldquo;`/`&rdquo;` in JSX text across `/pay`, `/pay/[invoice]`, `/admin/payments/inbox`, `/admin/payouts/runs`, `/admin/payouts/ad-hoc`), converted two `useEffect` dependency violations to `useCallback` (`/admin/payouts/tax-report`, `/admin/payouts/runs/[id]`). Fixed pre-existing stale assertion in `lead-reply-qa.test.ts` (doc was moved to `completed/`). Source-locked by `__tests__/admin/payment-admin-styling.test.ts` (26 green). `npx next lint` now reports zero errors._ |
