...
| **B** | **Photos/Images tab.** New `JobPhotoGallery` component + "Photos" tab. Reuses the existing files API/storage but filters to image types, renders a responsive thumbnail grid with a lightbox (click to enlarge, prev/next), drag-and-drop or button upload, caption + delete. Uploads tagged `section='photos'` so they're distinct from documents. | ⏳ |
