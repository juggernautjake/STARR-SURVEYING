...
> served its purpose:
>
> - [x] **#1 (critical)** `comparison.service.ts` pixel-space closure fix — `runMathematicalChecks` now takes `canvasScale` from `drawing.canvas_config.scale` (pixels per survey foot). `closure_misclosure_ft` divides pixel-space misclosure by the scale to recover real feet. `area_computed_acres` divides shoelace's px² result by scale² to get sq-ft, then / 43560 → acres. `closure_precision` (ratio) is scale-invariant and stays unchanged. The old code computed misclosure in pixels but the field name claimed feet — now the units match.
> - [x] **#2 (critical)** AI responses now shape-validated before the cast — `assertAnalysisShape` in `document-analysis.service.ts` checks both `LegalDescriptionAnalysis` and `PlatAnalysis` payloads: rejects non-objects, rejects 11 array fields that are not arrays, rejects `closure`/`notes` if non-string. Malformed AI output throws (caught upstream by the outer try/catch that already exists in the analyzeDocument flow, surfaced as an `error` field on the result row). Per-field Zod schema enforcement is a follow-up tracked inline.
