...
| **A-8** | Enable RLS on high-blast-radius tables + tests | 4 days | ✅ SQL shipped — `seeds/273_saas_rls_high_blast.sql` enables RLS on the financial-data tables (payroll / employees / receipts / wallet / subscriptions / invoices / rewards / org_invitations). Helper preserved from M-7; dropped after M-8. Pre-flight check fails loud if helper is missing. Per-table isolation tests against a real DB are the next slice. |
...
| **A-9b** | Auth refactor — middleware bundle check + signIn flow | 4 days | Deferred — needs deliberate single-shot focus (middleware change + signIn callback rewrite + feature flag for rollback). Substrate is in place (A-9a fills the JWT; route registry has requiredBundle; `bundle-gate.ts` evaluates access). |
