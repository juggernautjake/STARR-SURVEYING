...
- [x] COPILOT card surfaces Accept / Modify / Skip; Modify reopens a chat input pre-populated with the proposal so the surveyor can redirect. *— Slice 5 ships `CopilotCard` (mounted in `CADLayout`) with all three buttons + a per-card sandbox toggle that defaults to the proposal's `sandboxDefault` then to the store-wide `aiStore.sandbox`. Accept routes through `useAIStore.acceptHeadProposal(sandbox)` which calls `executeProposal` (tool registry + provenance + sandbox routing). Skip dequeues without executing. Modify is a Slice-6 stub that announces "Modify lands in Slice 6 — Skip and ask via chat for now"; the chat-input-pre-populated-with-proposal flow lands when the real AI adapter (Slice 6) replaces the mock proposer. Canvas paints a dashed amber ghost (`cad:copilotPreview` → `renderCopilotPreview`) for geometry-producing proposals; layer-op proposals show their result in the layer panel after Accept.*
