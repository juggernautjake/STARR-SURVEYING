...
- [x] LINE × ARC yields 0 / 1 / 2 candidates depending on geometry; cycling with ↓/↑ moves the canvas highlight — Slice 3 adds `lineCircleIntersections` and `lineArcIntersections` to `lib/cad/geometry/intersection.ts` (covered by 14 unit tests in `__tests__/cad/geometry/intersection.test.ts`: miss / tangent / two-point / vertical-line / off-center / arc-sweep-filter / boundary-angles). The dialog grows a Method picker (`LINE × LINE` / `LINE × CIRCLE` / `LINE × ARC`); Source B's accepted feature types switch automatically. The candidate region renders a clickable list with per-row "within both" / "extended A N ft" / "extended B N ft" badges; clicking a row sets `selectedIndex` so the canvas crosshair highlights it. Keyboard cycling (↑/↓) lands in Slice 7 along with the full multi-candidate workflow.
