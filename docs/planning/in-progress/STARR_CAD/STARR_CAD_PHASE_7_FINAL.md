...
- ~~Enrichment data auto-fills PLSS fields in title block~~ — deferred: PLSS doesn't apply to Texas (TX is a metes-and-bounds state, not a public-land state — no Section/Township/Range survey lattice). The functional equivalent — Texas survey-grant fields (abstract / survey-name / county) — already flows the *opposite* direction via `lib/cad/delivery/description-generator.ts` sniffers (lines 168-171) reading the surveyor-edited `titleBlock.notes` into structured fields for the legal description. Auto-filling `titleBlock.notes` from external parcel-CAD enrichment depends on the Phase 6 AI engine's enrichment pipeline; `lib/cad/ai-engine/enrichment.ts` keeps the PLSS placeholder fields at null pending that work. Revisit when Phase 6 AI engine ships.
