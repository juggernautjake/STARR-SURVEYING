...
**Date:** 2026-07-25
**Status:** ✅ **COMPLETE** — every action item shipped. Workstream A (slices 1–6), Workstream B (B1–B4), and the consolidation map (C1–C4) are all done and browser-verified except where noted below.

### Closing status (2026-07-25)

| Item | Outcome |
|---|---|
| Workstream A — draft model, `/variants` actions, EditFlow, DraftSaveBanner, tests | Shipped (slices 1–6) |
| Workstream B — preview/confirm route, chat UI, Q&A grounding, tests | Shipped (B1–B4) |
| Fold in `SystemSwitcher` (transpose/add, switch/rename/delete, level-up, build report) | Shipped (C1–C3); **panel retired** |
| Fold in `LibraryChat` Q&A → sheet chat | Shipped (B3) — the separate box now shows only to read-only viewers |
| Fold in `VariantToggle` vanilla/custom → EditFlow | Satisfied as specified: EditFlow owns the vanilla-vs-homebrew step **and** the quick sheet toggle was explicitly to be kept, so nothing is removed |
| Remove the orphaned `/level-up` route | Moot — no such page route exists (only the live API route) |
| Remove redundant on-sheet Foundation-builder panels | Shipped (C4), after first giving `/builder` an entry point from the sheet |

**Handed off, not dropped:** live browser QA of the **sheet chat** (Workstream B) needs a working AI key to exercise, so it joins the variant tracker's owed QA pass (`pending/CHARACTER_VARIANT_TRACKER_2026-07-25.md`) and the final walkthrough (`pending/DND_FINAL_QA_WALKTHROUGH.md`). Everything else in this doc was driven in a real browser via the dev-only harness at `/dnd/preview/edit-flow`.

**One deliberate non-deletion:** `app/dnd/_ui/SystemSwitcher.tsx` is no longer rendered anywhere but the file remains for one merge cycle, so QA can restore the old panel in a single revert if the consolidated flow misses something. Delete it after this branch merges and QA passes.
...
- **2026-07-25 — Consolidation slice C4: the on-sheet Foundation builders come off — after giving `/builder` a way in.** Checked the discoverability question first, and it was real: `/builder` had **no entry point from an existing character's sheet** (only `NewCharacterForm` at creation, and EditFlow's from-scratch path, which adds a *new blank slot in another system* — a different action). Removing the inline panels first would have left IG/PF2 characters with no builder at all. So EditFlow's root gained **"Rebuild this version step by step"** → `/dnd/characters/{id}/builder`, offered only on the **active** card because `/builder` always builds the active sheet. With that in place, `Dnd5eManualBuilder` / `PF2CharacterBuilder` / `IGCharacterBuilder` were removed from the sheet page (they live on `/builder`, which already dispatches all three systems). These *replace* a sheet's foundation, so sitting permanently under a finished character they were both redundant and the easiest way to overwrite a character by accident. `IGVanillaLibrary` **stays** — it is a read-only reference, not a builder; so does `CharacterBuildKit` (files/art/AI, not a Foundation builder). Fixed one more tofu glyph: `⬆` (U+2B06) rendered as a bare stroke, swapped for `⇡`, the arrow the sheet's Forms tab already proves renders in this face. 2 new render tests pin the rebuild entry present on the active version and absent otherwise. Browser-verified. typecheck 0, lint clean, **full suite 16,066 passing**.
