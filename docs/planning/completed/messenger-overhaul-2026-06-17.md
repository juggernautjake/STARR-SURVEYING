...
| **MX3** | New `lib/admin/use-draggable.ts` hook extracted from CalculatorModal. Wires into FloatingMessenger so the header is a drag handle that moves the panel and persists `{x, y}` to `admin/messenger/panel-position` in localStorage. The hook clamps to the viewport on resize, exits on `[data-no-drag]` / button / link / input targets, and exposes a `mounted` flag so SSR still renders the MX1 bottom-right anchor. Applying to DiscussionThreadButton is a quick follow-up — same hook, same pattern. ✅ shipped (messenger; discussion in MX4) |
| **MX4** | DiscussionThreadButton adopts the same `useDraggable` hook (distinct width/height/storage-key so users can park the two popups independently). Header tabs + close get `data-no-drag`. The messenger's group-chat creation (`new` view with multi-select contacts + optional group title) is already reachable from the always-visible sidebar's ✏️ button — kept that flow exercised by the source-lock. ✅ shipped |
| **MX5** | New `lib/admin/messenger-search.ts` ships pure `highlightSegments(snippet, query)` + `snippetAroundMatch(content, query, maxLen)` helpers. FloatingMessenger's `search` view now controls the query via `msgSearchQuery` state, shows a "N matches for <query>" summary, names the source conversation on every row, wraps the matching text in `<mark>` segments (with snippet trimming + ellipses), and differentiates "type more" / "no matches" empty states. ✅ shipped |
