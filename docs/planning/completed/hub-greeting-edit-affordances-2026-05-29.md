...
#### Slice 220 — Bigger resize grip + size badge on hover ✅ shipped
...
- **Done:** Grip grew from 18×18 muted-on-elevated to 28×28 accent pill (background `var(--theme-accent)`, color `var(--theme-accent-fg)`, soft shadow + `zIndex: 2` so the cell body can't render over it). Offset bumped from `bottom: 4 / right: 4` to `bottom: 6 / right: 6` so the grip sits inside the Slice-219 dashed edit ring without clipping. The size badge was rewritten — previously it only existed *during* an active drag; now it's always mounted while the handle exists (which means always in edit mode, because the cell only renders the handle when `resizeEnabled`). The idle badge is a subtle dark monospace chip pinned to the top-left corner (`top: 6 / left: 6`, `border-radius: 999`, `rgba(0,0,0,0.55)` background, `ui-monospace` family so digits don't jitter as they tick during resize). On drag-start the badge swaps to `badgeActiveStyle` — accent background + bigger shadow — and announces the moving target. `aria-live` flips between `'off'` (idle) and `'polite'` (active drag) so screen-reader users hear the live size only while the surveyor is actively resizing, not on every cell mount. The handle's `title` attribute now surfaces the current size (`Drag to resize · 3×3`) for hover discoverability. 14 vitest specs lock the source-level contracts (28×28 dimensions, accent background, nwse-resize cursor, ⤡ glyph, zIndex stacking, badge always-on render with idle WxH, badge top-left positioning, pill radius, monospace family, pointer-events: none, active-state accent swap, aria-live ternary) + 2 SSR render assertions (visible badge in idle render + accessible name with current size). 1043 hub specs green. `tsc` + `eslint` clean.
