...
> All slices P1–P9 shipped; seed 402 (190 built-in FS flashcards) applied to live Supabase
> (category-scoped after two schema blockers were found & fixed). Moved to `completed/`.
...
| **P5** | Add **Practice** + **Flashcards** tabs to the FS module page; section-read tracking + reading indicator; flashcard discovery on open | **DONE** — tabs landed in P3/P4. Section-read tracking shipped: `fs` route gained `POST action:'mark_section_read'` (idempotent upsert into `fs_section_progress`) + a `sections_read[]` field on the single-module GET; the page marks a section read on first view of each content tab and shows a "N/5 sections read" indicator. tsc + lint clean. Discovery-on-open now wired too: `fs` route gained `POST action:'discover_module_cards'` (mirrors `progress/route.ts` — upserts the module's built-in cards into `user_flashcard_discovery`, idempotent on `user_email,card_id`), fired once when the module page mounts. No-op until P7 authors cards, but ready. **P5 fully shipped.** tsc + lint clean. |
| **P6** | Wire module quiz pass → `fs complete_quiz` unlock (`QuizRunner.onComplete`) | **DONE** — `QuizRunner` gained an optional `onComplete(summary)` fired once on grade; the FS module page passes `handleQuizComplete` → `POST .../exam-prep/fs {action:'complete_quiz', module_id, quiz_score}` then refetches so the pass flips `fs_module_progress` and unlocks the next module (route handler at `fs/route.ts:182` already existed). tsc + lint clean. |
| **P7** | Author FS flashcards (~15–25/module via `m<N>.json` + `gen_fs.js`), seed + apply | **IN PROGRESS** — Module 1 authored: 20 cards in `fs/m1.json` `flashcards[]` (derived from its vetted key_topics/formulas). New reproducible generator `gen_fs_flashcards.js` emits idempotent `seeds/402_fs_prep_flashcards.sql` (tag `fs-flashcards`/`fs-m<n>`, category `fs:m<n>`, deterministic UUIDs, DELETE-then-INSERT). **Not yet applied to live** — awaiting go-ahead before writing to prod Supabase. **DONE — 190 cards authored AND applied to live (2026-07-06).** All 10 modules in `seeds/402`. **Two live-schema blockers found & fixed on apply:** (1) `difficulty_level` CHECK domain is `beginner/intermediate/advanced/expert` — mapped easy→beginner, medium→intermediate, hard→advanced; (2) `flashcards.module_id` FKs `learning_modules` (a different course), not `fs_study_modules` — so FS cards are **scoped by `category='fs:<fs-module-uuid>'` with `module_id` NULL**. Wired the surfacing to match: flashcards GET route + user-card POST accept `category`; `FlashcardsPanel` fetches/creates by `category=fs:<moduleId>`; `discover_module_cards` queries by category. Verified live: 190 rows, correct per-module counts, category query returns each module's cards. |
| **P8** | Content spot-check (sections already rich — confirm, patch only if a gap surfaces) | **DONE** — confirmed all 10 modules in `seeds/368_fs_prep_buildout.sql` carry all five lesson sections (overview/concepts/formulas/examples/tips each appear exactly 10×) and the seed is ~768 KB of rich content. No gap surfaced → no patch needed. Locked with a guard test in `__tests__/learn/fs-prep-flashcards.test.ts`. |
| **P9** | CSS (`fs-module__practice`, `fs-module__flashcards`), tests, verify end-to-end | **MOSTLY DONE** — panel CSS already exists in `app/admin/styles/AdminLearn.css` (`fs-practice__*`, `fs-flash__*`; verified). Automated tests shipped: `__tests__/learn/fs-prep-flashcards.test.ts` (8 tests) source-locks seed 402 (190 cards, per-module counts, idempotency, deterministic UUIDs, namespacing) and the P5/P6 route+page+QuizRunner wiring. tsc + lint + vitest green. Seed now verified live; the category-scoped fetch path returns each module's cards (checked via node-pg). Data-path e2e confirmed. |
