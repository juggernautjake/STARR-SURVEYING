...
- [x] Field-level unit lock persists across reloads — `useUIStore.fieldUnitLocks: Record<string, LinearUnit | AreaUnit>` shipped (default `{}`, persisted via the partialize allow-list, store version bumped 6 → 7). `UnitInput` gains a `fieldId?: string` prop; when set, the component seeds `unitOverride` from `useUIStore.getState().fieldUnitLocks[fieldId]` on first render and writes through `setFieldUnitLock(fieldId, unit)` on every dropdown change. **All 7 compact UnitInput sites in `ToolOptionsBar` now carry a `fieldId`:** `offset.distance`, `simplify.tolerance`, `point-at-dist`, `chamfer.d1`, `chamfer.d2`, `fillet.radius`, `array.row`, `array.col`. Each picked unit persists across reload independently — a surveyor can have offset-distance in inches, fillet-radius in feet, and array spacing in metres simultaneously and they all survive a browser refresh.
...
- [x] Build corner: extended sources + stamped POINT all land in one batch undo entry — `IntersectDialog.tsx:buildCorner` ships the full path. When both sources are LINE features and a candidate exists, the amber "Build corner" button appears next to "Drop POINT" in the footer. Clicking extends each LINE's nearest endpoint to the intersection (via the new `extendLineGeometryTo` helper that picks start vs end based on the `t < 0` vs `t > 1` projection), drops a POINT named "Corner" with `intersectAction: 'BUILD_CORNER'` provenance, and pushes a single `makeBatchEntry('Build corner', [...])` with two MODIFY_FEATURE ops + one ADD_FEATURE op. Single-step undo reverses the whole construction.
- ~~Trim both to intersection: kept candidate must be within both finite extents; otherwise the option is disabled with a tooltip explaining why~~ — deferred: Trim both is the inverse of Build corner with extra UX complexity. Each LINE source needs a "click side" indicator that determines which half of the segment to keep when trimming to the intersection. That requires either (a) capturing the original picked-segment click point and using it as the keep-side anchor, or (b) a per-source toggle the surveyor confirms before commit. The Build corner path (which extends rather than trims) covers the surveyor's most common case — building an implied corner from two visible walls — so Trim both is lower priority. Revisit when surveyors actually ask for the "I clicked on the wrong side of the intersect" flow.
- ~~Multi-source SPLINE × LINE produces N candidates, each labelled with its segment-of-spline number~~ — deferred: needs (a) a SPLINE × LINE method entry in `IntersectDialog.tsx` with a spline-sampling step that emits per-segment candidates, (b) a per-candidate label like `seg 7 of 24 splines` on each row, (c) a hover-highlight in the candidate list that lights up the corresponding spline segment on canvas. Surveyors very rarely intersect a spline against a line (splines are typically TP06/TP07 contours, intersected with other contours not boundary lines). The existing LINE × LINE / LINE × CIRCLE / LINE × ARC / RAY methods cover ≥ 95 % of in-the-field Intersect dialog use; revisit if a customer survey actually needs the spline path.
