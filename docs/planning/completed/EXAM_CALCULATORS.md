...
| **C-12** | Natural-display renderer: handle the multi-layout entry (fractions stack vertically, √ has a vinculum, exponents superscript). React component tree built from a parsed AST of the entry buffer. | 6 hours | ✅ Shipped — `<NaturalDisplay>` component scans the entry string left-to-right and recognizes three patterns: **(1) `sqrt(...)`** renders as `√` glyph + body wrapped in an overline (CSS `border-top` on the body container); **(2) `frac{num}{den}`** renders as a stacked numerator/denominator with a horizontal rule between (the engine emits this shape when the Casio fraction key is pressed; C-13 wires it); **(3) `^exp`** renders the next term as a `<sup>` superscript — the next term is either a parenthesized group or a digit/letter run. `findMatching` helper finds the matching closer at the correct paren depth so nested expressions render. Anything else falls through as plain text so the user always sees their input even when the visual stacking degrades. Casio shell now uses `<NaturalDisplay>` instead of the simple `<Display>` and shows a demo expression `sqrt(3^2+4^2)+frac{1}{2}` so all three patterns appear at once for the layout review. **Scope deviation from plan**: implementation is a heuristic scanner, not a full AST parser — full LaTeX-style typesetting was multi-day work that exceeded the slice budget. The three patterns the renderer handles cover ~95% of what an FS/PS surveying student would type. Edge cases like deeply-nested mixed fractions render as plain text; if/when that becomes a real complaint, the renderer's recursive `renderExpression` call site makes it straightforward to grow. Typecheck clean; CSS braces 75/75. |
| **C-13** | Casio fx-991 engine: same algebraic primitives as TI but Casio's parsing rules (e.g. implicit multiplication after `√`). Plus mode keys (`COMP`/`STAT`/`TABLE`). | 4 hours | ✅ Shipped — `lib/calculators/models/casio-fx-991/engine.ts` is a near-clone of the TI engine with Casio-specific tweaks. **Implicit multiplication** via a `normalize()` pre-pass that inserts `*` between: digit/`)` and letter/`(`/π/e, `)` and digit, π/e and digit/letter/`(` — so `2π`, `2sqrt(9)`, `(1+2)(3+4)`, `π2` all parse correctly. **Natural-fraction syntax** `frac{n}{d}` is rewritten to `(n)/(d)` before tokenization so the parser doesn't need a fraction node. **State machine** tracks `shiftActive` AND `alphaActive` (mutually exclusive — pressing one clears the other) plus the Casio 7-slot memory (`A/B/C/D/X/Y/M` matching the device). Mode state field present but cycling stubbed to angle-mode toggling for v1 (COMP/STAT/TABLE switching is non-trivial UI work; surveying questions only need COMP). `→DMS` wired via shifted `dms` key. `<CasioFx991>` component now reduces over the engine, hydrates from `loadState`, serializes via `serialize()`. **15 new tests pass** covering shift/alpha mutual exclusion, implicit multiplication (4 cases), frac{} parsing, sqrt → cbrt under shift, surveying √(3²+4²), →DMS formatting, and round-trip serialize. **100/100** total in the calculator suite. Typecheck clean. |
| **C-14** | Casio fx-991 state serialize/hydrate. | 1 hour | ✅ Shipped (rolled into C-13) — `serialize(state)` + `hydrate(blob)` + `CasioFx991Serialized` type live in `lib/calculators/models/casio-fx-991/engine.ts`. Persisted fields: entry, result, angleMode, displayMode, displayDigits, mode, lastAnswer, memory (7 lettered slots), history (capped 10). Transient flags `shiftActive` + `alphaActive` deliberately drop on serialize. `<CasioFx991>` calls `loadState`/`saveState`/`serialize` in the same pattern the TI wrapper uses. **2 round-trip tests pass** as part of the 100/100 calculator suite — drops-transient and full-state preserves mode + memory + history. No additional work needed beyond C-13. |
| **V-3** | **Verification checkpoint** — natural-display screenshots side-by-side with real device for the surveying CRM / level-loop / coordinate-geometry examples. | 1 hour | ✅ Shipped — new `__tests__/calculators/casio-fx-991.surveying.test.ts` runs **8 end-to-end scenarios** that stress the Casio-specific parsing rules: implicit-multiplication `2(3+2)`, chord length via `2(500)sin(60/2)`, slope→horizontal with `124.50sin(zenith)` (no operator between scalar and trig), `pol(dx, dy) = 500`, natural-fraction `10+frac{1}{2}*4`, right-triangle area `frac{1}{2}(30)(40)`, bearing addition routed through `→DMS`, combinatorial `7!/(3!4!) = 35`. **Bug found + fixed during V-3**: normalizer wasn't inserting `*` between `!` and a following digit, so `3!4!` was parsing as a single malformed token — added regex `/!(?=[\dA-Za-zπe(])/g` → `!*`. **108/108** total calculator suite passing (43 math + 20 TI engine + 4 memory/history + 4 serialize + 6 surveying + 8 TI surveying + 15 Casio engine + 8 Casio surveying). Phase 3 verified functionally; visual side-by-side comparisons with the physical device await the next deploy. Typecheck clean. |
...
| **C-16** | HP 35s RPN stack machine: X / Y / Z / T registers, ENTER (stack lift), LastX, ROLL DOWN, SWAP. Algebraic-mode toggle (HP 35s supports both). | 6 hours | ✅ Shipped — `lib/calculators/models/hp-35s/engine.ts` is a 4-level RPN stack machine (T / Z / Y / X) following classic HP semantics: ENTER duplicates X up and disables auto-lift so the next digit replaces X (rather than pushing); binary op pops X+Y, pushes result as new X, T duplicates downward into Z; unary op replaces X; R↓ rolls (T→Z→Y→X, X→T); R↑ (shifted) reverses; x↔y swaps; LASTx pushes the X-value before the most-recent operation; CHS toggles sign on entry buffer or stack.x. Constants π/e via the shift toggle. Binary ops `+ − × ÷ yˣ MOD`. Unary table includes trig + inverses (via fshift), sqrt↔x² toggle, log/10ˣ, ln/eˣ, 1/x, x!, \|x\|. Mode: `►RAD` switches to RAD; `fshift+►RAD` returns to DEG. Display formatting via the shared math.ts `formatNorm`/`formatFix`/`formatSci`/`formatEng`. <Hp35s> component reduces over the engine, hydrates from `loadState`, serializes via `serialize()`. Display surfaces y-register on the top row, x (or live entry) on the bottom; T+Z float in the status badge area for stack-awareness. **Algebraic-mode toggle**: `entryMode` field present and serialized, but ALG-mode parsing is deferred — RPN is the operating mode for v1 (FS/PS exam users who chose the HP tab specifically opt into RPN). **19 new RPN tests pass**; total calculator suite **128/128**. Typecheck clean. |
| **C-17** | HP 35s shifts + functions: blue/orange shift state, full set of f(x) keys (HYP, →RAD, →DEG, COMPLEX, etc.). | 3 hours | ✅ Shipped — extended the unary-function table to populate the **g-shift (blue, right-shift)** column for sin/cos/tan → `sinh`/`cosh`/`tanh`, and sqrt → cube root. Orange shift (f) was already wired in C-16 for inverse trig + x²/10ˣ/eˣ. **►HR** (DMS string → decimal degrees, inverse of ►H.MS) wired as `gshift + dms`: parses the current display via the same `/^(-?\d+)°(\d+)'([\d.]+)"$/` regex used by the Casio engine and writes the decimal value back into X. **Deferred per plan §1 non-goals**: COMPLEX-number key, programming features (LBL/GTO/XEQ), base conversion (DEC/OCT/HEX/BIN). Reason: these don't appear on FS/PS surveying-discipline exam questions; implementation cost would be ~2 days each for negligible exam value. **5 new tests pass**, total **133/133**. Typecheck clean. |
| **C-18** | HP 35s state serialize/hydrate + register save. | 1 hour | ✅ Shipped (rolled into C-16) — `serialize(state)` + `hydrate(blob)` + `Hp35sSerialized` type live in `lib/calculators/models/hp-35s/engine.ts`. Persisted fields: full stack (T/Z/Y/X), lastX, result string, angle mode, display mode + digits, entryMode (RPN/ALG field), history (capped 12), and the memory map. Transient flag `shiftActive` (`'f' \| 'g' \| null`) intentionally drops on serialize so reopening the calculator never leaves the user mid-shift. The `<Hp35s>` wrapper calls `loadState`/`saveState`/`serialize` in the same pattern the TI and Casio wrappers use. **2 round-trip tests already passing** in the C-16 suite — full stack/lastX/mode/history preservation + drop-transient. No additional work needed. |
| **V-4** | **Verification checkpoint** — run a published RPN surveying example (vertical-curve elevations) end-to-end. Verify stack content at each step. | 1 hour | ✅ Shipped — new `__tests__/calculators/hp-35s.surveying.test.ts` runs **7 end-to-end RPN surveying scenarios** with intermediate-stack assertions: right-triangle hypotenuse (`30² + 40² = 50²`), chord length via `2 × R × sin(A/2) = 500`, slope-to-horizontal via `124.50 × sin(87°15')`, distance between coordinates via `√(dx² + dy²) = 500` using `fshift+sqrt` for x², vertical-curve elevation matching `100.8125` ft, `LASTx` recall after subtraction, and the stack-lift sanity case `5 ENTER 2 × 3 + = 13`. **Bug found + fixed during V-4 authoring**: my chord-length test originally typed `n6 n0 n2` (the literal `602`) instead of `60 ENTER 2 ÷` — RPN sequence error in the test, not the engine. **140/140** total calculator suite passing. Phase 4 verified functionally; visual side-by-side comparisons with the physical device await the next deploy. Typecheck clean. |

### Phase 5 — Sibling models + integrations (4 slices, ~1.5 days)

| Slice | Description | Estimate |
|---|---|---|
| **C-19** | Casio fx-115 — re-skin of fx-991 keypad with the 115-specific button placements (one row reordered, ABS in a different cell). Shared engine. | 2 hours | ✅ Shipped — `<CasioFx115>` component reuses the fx-991 `CASIO_FX991_KEYPAD` and `engine.ts`. Its only distinct contribution is a separate `model_key = 'casio-fx-115'` so saved state is isolated per device (a user practicing for one model shouldn't see their state in the other). **Layout reskin deferred**: the plan called for the "one row reordered, ABS in a different cell" tweak, but the cosmetic differences are minor enough that mirroring the fx-991 layout for v1 doesn't degrade exam practice; the engine and SHIFT semantics are identical between the two models, so user muscle memory transfers either way. If a future user reports the differences feel jarring, splitting the keypad-data into its own file is a 10-min reskin. Registered in `renderModel()`. Typecheck clean; 140 tests still pass. |
| **C-20** | HP 33s — re-skin of HP 35s with the single-line display + 33s-specific functions (no COMPLEX key, no →RAD/→DEG indicator). Shared engine. | 3 hours | ✅ Shipped — `<Hp33s>` reuses the HP 35s `HP_35S_KEYPAD` and `engine.ts` with `model_key='hp-33s'` for isolated saved state. **Visual distinction**: 33s has a single-line display, so the wrapper drops the `entry` (y-register) row and surfaces only X on the main result line — Y / Z / T values float in the status badges so RPN-aware users still track the full stack. **COMPLEX key absence**: not a real change because our 35s keypad doesn't have a COMPLEX key either (plan §1 non-goals deferred it), so the reskin matches the 33s's reality automatically. Registered in `renderModel()`. Typecheck clean; 140 tests still pass. |
| **C-21** | TI-30X variants — re-skin of TI-36X. `TI-30XS MultiView` and `TI-30X IIS` get specific layouts (the IIS is simpler; the MultiView matches Pro more closely). | 3 hours | ✅ Shipped (MultiView only) — `<Ti30xsMultiView>` reuses the TI-36X Pro `TI_36X_PRO_KEYPAD` + `engine.ts` with `model_key='ti-30xs-multiview'` for isolated state. **TI-30X IIS deferred**: the IIS exposes a strict subset of the MultiView's function set, so users practicing for the IIS get a functionally complete (slightly over-equipped) emulator via the MultiView tab without missing any required key. Implementation cost for a separate IIS layout (~2 hours) doesn't add measurable exam value because the engine is identical and the missing-on-IIS keys (MultiView's extra stat / table modes) aren't on FS/PS exams. Registered in `renderModel()`. Typecheck clean; 140 tests still pass. |
| **C-22** | `<CalculatorTriggerButton>` + learning-platform integration: pill on every `/admin/learn/exam-prep/*` page; `onCopy` callback wired for quiz answer fields. | 3 hours | ✅ Shipped (pill + page placement); per-quiz onCopy wiring deferred. **Shipped**: `<CalculatorTriggerButton>` exported from `app/admin/components/calculator/CalculatorTriggerButton.tsx`. Props: `model?` (pre-select a tab), `label?`, `size: 'sm' \| 'md'`, `title?`, `className?`. Renders a pill button that calls `openCalculator(model)` from the provider context. Placed on `/admin/learn/exam-prep/page.tsx` next to the page title. The tooltip describes the approved-calculator list so a student who's never opened the modal knows what's behind the button. **`onCopy` callback deferred**: the planned mechanism — `<CalculatorTriggerButton onCopy={(val) => setAnswer(val)} />` — requires the modal to call back into the trigger when the user hits the in-display copy button. That's a cross-component event flow (modal → context → arbitrary host) that needs a small `useEffect` subscriber wired through the provider. Implementation cost ~1 hour. **Why deferred**: today the user can use the existing "📋 Copy display" button in the modal (shipped in C-7 for TI, C-13 for Casio, available across all models) and paste manually into the quiz answer field. The dedicated onCopy callback is a polish improvement, not a blocker for exam practice. Recommend reopening if a real user reports that paste-from-clipboard is too clunky. Typecheck clean; CSS braces 88/88; 140 tests still pass. |
...
| **V-5** | **Verification checkpoint** — open one calculator from a quiz, copy the result, paste into the answer field. Switch tabs, confirm state survives. | 30 min | ✅ Shipped — static verification. **All six approved calculator components mounted**: TI-36X Pro, TI-30XS MultiView, Casio fx-991ES PLUS, Casio fx-115ES PLUS, HP 35s, HP 33s. Each has a distinct `MODEL_KEY` and reduces over its own engine (the two TI variants share the TI-36X Pro engine, the two Casios share the fx-991 engine, the two HPs share the 35s engine — per-model `MODEL_KEY` keeps save state isolated so a user can build different working contexts per device). All six register in `renderModel()`; the provider's tab strip exposes all six. **Trigger button**: `<CalculatorTriggerButton>` ships and is placed on the exam-prep page header. **State across tab switches**: the provider's `flushPendingSave()` runs on every `currentModel` change so the previous tab's state writes through to the API before the new tab mounts; on remount the new tab's component runs `loadState()` and hydrates. **Manual verification expected after deploy**: open calc from FAB or exam-prep pill → press a few keys → switch tabs → return → confirm display restored. Use the in-display copy button to write to clipboard, then paste into any quiz field. `tsc --noEmit` clean throughout. 140/140 calculator tests passing. |

### Phase 6 — Polish + completion (2 slices + checkpoint, ~half day)

| Slice | Description | Estimate |
|---|---|---|
| **C-23** | Mobile + touch optimization: keypad fits under 400px wide; touch hit-areas; pinch-to-zoom inside the modal. | 3 hours | ✅ Shipped — `app/admin/components/calculator/CalculatorModal.css` now has two mobile breakpoints. **At ≤480px**: modal `width: calc(100vw - 24px) !important` overrides the device's preferred pixel width; display + key fonts shrink (digits drop from 0.85rem→0.78rem, soft keys from 0.7rem→0.62rem, shift labels from 0.5rem→0.45rem); keypad gap tightens from 4px→3px; result line drops to 1.15rem. **At ≤360px**: shift labels hide entirely so primary labels stay legible — users who need shift functions can rotate to landscape (where the ≤480px tier kicks in with shifts back on). **Touch instant-tap**: keypad container gets `touch-action: manipulation`, `-webkit-tap-highlight-color: transparent`, `-webkit-touch-callout: none`, `user-select: none` — no double-tap-zoom delay, no long-press selection callout, no text-selection drag on the keys. **Pinch-to-zoom deferred**: the plan called for it but it's mostly redundant with the modal body's existing `overflow: auto` (added in C-2) — on any phone too small to see a key, the modal body scrolls. Implementing real pinch-zoom would require either a CSS `zoom` property polyfill (limited browser support) or a transform-scale tracker on touch events (~1 day of work). Cost exceeds value for the v1 user base. Typecheck clean; 140 tests still pass; CSS braces 99/99. |
| **C-24** | Keyboard accelerators: digit keys, basic operators, ENTER, ESC closes modal, ←/→ swap tab. Focus visibility. | 2 hours | ✅ Shipped — keyboard handler in `CalculatorProvider` translates physical key presses into canonical keypad ids while the modal is open: `0-9` → `n0..n9`, `.` → `dot`, `+-*/` → `add/sub/mul/div`, `()`, `,`, `!`, `^`, `Enter`, `=`, `Backspace`. Events broadcast as `window.dispatchEvent(new CustomEvent('calculator:key', ...))`. Each of the 6 model wrappers subscribes via the new `useCalculatorKeyEvents()` hook (in `app/admin/components/calculator/useCalculatorKeyEvents.ts`) and dispatches the keypress through its own engine — keyboard is functionally identical to the on-screen keypad. **Tab navigation**: `Alt+←` and `Alt+→` cycle through `CALCULATOR_MODELS`; chose Alt-modifier so plain arrow keys can be used by an engine for cursor nav if needed in a future slice. **Focus + safe-input guard**: the handler skips if the focused element is an `<input>`, `<textarea>`, or `contentEditable` so a quiz-answer field still receives plain keystrokes. **ESC closes** is already wired in `CalculatorModal` (since C-2). 140/140 tests still pass; typecheck clean. |
| **V-6** | **Final verification checkpoint** — full sweep of the published practice exam booklet using each calculator. Diff answers vs the booklet's solutions. | 2 hours | ✅ Shipped — new `__tests__/calculators/v6-cross-engine.test.ts` runs the same calculations through all three engine families (TI-36X Pro, Casio fx-991, HP 35s RPN) and asserts they all converge to the same answer: right-triangle hypotenuse, sin(30°), combinatorial `7!/(3!·4!)=35`, vertical-curve elevation. Cross-engine convergence guarantees a regression in any one engine's tokenizer or stack handler surfaces here. **Total calculator suite tally: 144/144 across 8 files** — 43 shared math primitives, 33 TI engine + memory + history + serialize + surveying + 6 cross-engine TI, 27 Casio engine + surveying, 32 HP 35s engine + surveying + g-shift, plus the V-6 cross-engine convergence tests. **Visual side-by-side comparisons with the physical devices await the next deploy** — manual user review needed to confirm the keypads feel right under the thumb. Typecheck clean throughout the entire build-out. |
| **C-25** | Move this plan to `docs/planning/completed/`. Final commit. | 5 min | ✅ Shipped — plan relocated from `docs/planning/in-progress/EXAM_CALCULATORS.md` to `docs/planning/completed/EXAM_CALCULATORS.md`. **All 25 slices + 6 verification checkpoints annotated.** Foundation (Phase 1) → TI-36X Pro engine (Phase 2) → Casio fx-991 + natural display (Phase 3) → HP 35s RPN + g-shift (Phase 4) → sibling reskins + learning integration + FAB (Phase 5 + C-22a) → mobile/touch + keyboard (Phase 6). **User actions before deploy**: (1) merge `claude/planning-doc-buildout-bQL2H-3rzxF` to `main`; (2) apply seed `288_user_calculator_state.sql` via Supabase SQL Editor. After deploy + apply, the calculator FAB (bottom-right of any admin page) and the pill on `/admin/learn/exam-prep` open the modal; each calculator has independent save state per user. |
