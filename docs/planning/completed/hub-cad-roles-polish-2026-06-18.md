...
| **W9b** | Consolidated `drawings-hub` widget — absorbs drawings + drawings-in-progress + recent-drawings. Mine/All scope toggle at medium+; per-row deep-link to `/admin/cad?drawing=<id>`. Pure helpers `limitForBucket` + `formatRelative` exported. ✅ shipped |
| **W9c** | Consolidated `money` widget — absorbs my-pay + monthly-revenue + outstanding-invoices. Fans out four parallel reads (`/payroll/employees`, `/payroll/payout-log`, `/reports?metric=monthly-revenue`, `/billing/invoices`); the 401/403→empty `readOrSkip` helper means non-finance users still see their pay without fake error chrome for revenue / invoices. Size-relative variants per W5: tiny=last paycheck, small=Pay only, medium=Pay+Revenue 2-col, large=3-col, xlarge=3-col + invoice list. Pure helpers `fmtUSD` + `fmtCents` + `moneyLayoutForBucket` exported. ✅ shipped |
| **W9d** | Consolidated `learning-stack` widget — absorbs class-assignments + flashcards-due + recommended-lessons. Three parallel reads (`/learn/assignments`, `/learn/flashcards?due_count=true`, `/learn/recommended`); 401/403→empty per the W5 pattern. Size-relative variants: tiny=total stack count, small=Assignments only, medium=Assignments+Flashcards 2-col, large=3-col (limit 6), xlarge=3-col (limit 10). Pure helpers `totalLearningCount` + `learningLayoutForBucket` + `assignmentHref` + `recommendedLessonHref` exported. ✅ shipped |
| **W9e** | Consolidated `field-pulse` widget — absorbs team-status + vehicles-status + equipment-out + low-consumables. Four parallel reads (`/team/status`, `/vehicles`, `/equipment/today`, `/equipment/consumables`); 401/403→empty per the W5 pattern. Size-relative variants: tiny=aggregate signal count, small=Team only, medium=Team+Equipment 2-col, large=2×2 four-tile grid (counts only), xlarge=2×2 with per-tile row lists. Pure helpers `totalFieldPulseCount` + `fieldPulseLayoutForBucket` + `isLowConsumable` exported. ✅ shipped |
| **W10** | CAD area-measuring tool revamp: sticky `AreaMeasureHUD` at bottom-right with live perimeter / sq ft / acres readouts; Undo (Backspace) / Clear / Close & log buttons; "Snap to nearest foot" toggle wired through a new `cad:setAreaSnap` window event the CanvasViewport listens for (rounds vertex clicks before pushing). Vertex hit targets bumped from 4 px → 7 px with a 1.5 px white outline ring; first vertex gets an extra emphasis ring once the polygon can close. Pure helpers `summarizeAreaMeasurement` + `snapToFootGrid` exported from a new `lib/cad/geometry/area-measurement.ts` and locked under vitest. ✅ shipped |
| **W11** | CAD free-hand drawing tool: new `DRAW_FREEHAND` ToolType + ToolBar entry with Solid / Dashed / Dotted / Smoothed variants. The Smoothed variant flips `toolState.freehandSmooth`, which runs Chaikin's corner-cutting algorithm on the captured stroke before commit. Stroke style inherits from the existing `toolState.drawStyle` (color / opacity / line weight / line type). CanvasViewport gets a `freehandActiveRef` + pointer wiring: mouseDown arms the drag + seeds the first point; mouseMove accumulates points respecting `freehandMinSpacingFt` (default 0.5 ft); mouseUp decimates + optionally smooths then commits a POLYLINE feature via `createFeature` + `makeAddFeatureEntry`. Live preview renders the captured path through the existing `drawingPoints` render loop in the surveyor's chosen color / opacity / weight. Pure helpers `decimateByMinSpacing`, `chaikinIteration`, `chaikinSmooth` exported from a new `lib/cad/geometry/freehand.ts` and locked under vitest. ✅ shipped |
