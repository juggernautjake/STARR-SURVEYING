...
| **S5** | Learning. `quiz-history` grows: medium+ surfaces an avg-percent + passed + failed chip strip, large+ draws a small score-trend sparkline (oldest → newest, capped at 12 points, pass threshold marked with a dashed line). `roadmap-progress` grows: large+ surfaces a per-module mini-progress strip (title + percent + thin progress bar; success-tint at 100%). Pure helpers `summarizeAttempts`, `tintForPct` + `moduleCapForBucket` exported. `toRoadmap` now keeps the per-module list on the Roadmap rollup. DEFERRED: `class-assignments`, `flashcards-due`, `recommended-lessons` (consolidated into `learning-stack` — W9d). ✅ shipped |
| **S6** | Personal prefs + misc. `sun-calculator` grows: medium+ surfaces a "↑/↓ Next sunrise/sunset in 1h 23m" countdown line; large+ adds a daylight-progress bar (warning → accent gradient, 0→100% across today's daylight window). Pure helpers `nextSunEvent`, `daylightProgress`, `formatCountdown` exported. DEFERRED: `pinned-pages`, `quick-actions` (both already self-measure cell pixels and fill the actual grid cell rather than a bucket cap), `bookmarks`, `contacts` (per-bucket caps + grid cols already wired), `daily-briefing` (size-aware section caps already wired), `recent-activity`, `activity`, `approvals` (already follow the consolidated W5 / W8 / W9 pattern). ✅ shipped |
| **S7** | Cross-widget shell polish. `WidgetEmpty` + `WidgetError` now self-measure via the shared `useElementSize` hook and pick a compact variant when the rendered cell is too small to fit the full layout: tiny (< 120 × 80 px) collapses to an icon + 1-line title (Empty) or ⚠ + retry pill (Error); small (< 220 × 140 px) keeps the headline but drops the description / message and the secondary CTA; default renders the full layout. `WidgetSkeleton` already adapts via the `rows` prop + honors `prefers-reduced-motion`; locked under the spec so a future cleanup can't regress. Pure helpers `pickEmptyVariant`, `pickErrorVariant` exported (mirrored thresholds so empty + error collapse together). ✅ shipped |
