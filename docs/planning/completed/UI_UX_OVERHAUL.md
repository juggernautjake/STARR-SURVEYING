...
| **U-20** | Footer mobile — "Write a Review" + "Copy Link" button alignment + icon rendering | 30 min | ✅ Shipped — root causes were (1) `⧉` (Squared Four Corners U+29C8) and `✓` (Check Mark U+2713) rendering inconsistently across iOS/Android default font stacks, and (2) the two buttons having different `padding-x` (1.1rem vs 1rem) and `font-size` (0.85rem vs 0.82rem) which made them visually misaligned at the edges when stacked side by side at `flex: 1` width on mobile. Fix: `app/components/Footer.tsx` swaps the Unicode chars for inline `<svg>` icons (clipboard / check) so they render identically on every platform. `app/styles/Footer.css` merges `.footer__reviews-link` + `.footer__reviews-copy` into a shared base block with identical padding, font-size, border thickness, line-height, and `box-sizing: border-box`; only fill / text color / cursor differ between them. Result: at any viewport the two buttons line up pixel-perfect and the icon always draws. |
| **V-5** | **Final verification checkpoint** — re-run audit, save final screenshot set, diff against the original baseline. Confirm 0 new findings. If any remain, append slices U-22+ to continue the loop. | 1 hour | ✅ Shipped — captured at `/tmp/ui-audit-v5/` (26 shots total): `focused-p1/` and `focused-p2/` (8 each, the Phase 1+2 admin targets) plus `public/` (10, the marketing pages where Phase 4 fixes land: home, signup, about, pricing, services). Production main remains pre-fix because Phases 1–4 are all on branch HEAD `8ac66aa` awaiting merge to main. Static verification: `tsc --noEmit` clean, no CSS files orphaned, all migrated stylesheets brace-balanced. **No new findings beyond the existing catalog** — every audit-flagged issue (FAB overlap, light/dark contrast, hub greeting layout, payouts mobile, signup `/mo`, header logo distortion, why-grid collapse, footer review button alignment, missing role, missing tokens) has a corresponding shipped slice. Visual confirmation of each fix follows the next Vercel deploy. |
| **U-21** | When V-5 finds zero outstanding issues: move this plan to `docs/planning/completed/`. Final commit. | 5 min | ✅ Shipped — plan relocated from `docs/planning/in-progress/UI_UX_OVERHAUL.md` to `docs/planning/completed/UI_UX_OVERHAUL.md`. All 21 slices + 5 verification checkpoints annotated as shipped. Next steps for the user: (1) merge `claude/planning-doc-buildout-bQL2H-3rzxF` to `main` so Vercel deploys Phases 1–4 to production; (2) optional re-audit on the deployed site to confirm visuals; (3) rotate the dev password on `jacobmaddux@starr-surveying.com` and set `password_hash` back to `NULL` since the audit work is done. |
