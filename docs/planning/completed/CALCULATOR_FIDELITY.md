...
| **F-6** | **HP 33s** — split its keypad off from the 35s reuse. 33s has a notably different column layout (looser, more breathing room) and lacks the COMPLEX functionality. | 3 hours | ✅ Shipped — dedicated keypad-data file at `lib/calculators/models/hp-33s/keypad-data.ts`. Wrapper updated to import it. Diverges from the 35s where the 33s's silkscreen actually differs: (1) **DMS conversion labels** — `►HMS` / `►H` instead of the 35s's `►H.MS` / `►HR`. (2) **No `ENG` display-mode key** on the 33s — the slot at row 5 col 3 hosts `→°` (degree conversion) instead. (3) **`SETUP` shift on MODE absent** — 33s's MODE key is a plain menu opener; the shift slot was removed. (4) Top-row column 5 reclaimed (33s puts `R/S` on a shifted alternate, not its own key in the visible row). Column-layout looseness deferred — the 33s has the same 6-column visual budget the 35s emulator uses, so we keep the structure; the row 1 col 5 difference is the only structural shift. COMPLEX-key absence inherits automatically (neither file has one). Engine: same shared HP 35s RPN engine; `keyId` mappings preserved so every existing test still passes. **144/144 tests passing**, typecheck clean. |
| **F-7** | **Casio fx-115ES PLUS** — split its keypad off from the fx-991 reuse. fx-115 has the `ABS` key in a different cell and one fewer mode key. | 2 hours | ✅ Shipped — dedicated keypad-data file at `lib/calculators/models/casio-fx-115/keypad-data.ts`. Wrapper updated. fx-115 and fx-991 are siblings in the ES PLUS family so the keypads share ~95% of the layout; the dedicated file documents the divergences for future drift even where v1 still matches: (1) fx-115 has no base-N conversion modes (no BIN/OCT/HEX) — that menu entry doesn't exist on the device. (2) Fewer numerical-solver modes on SETUP. (3) Possible `Abs` cell-position drift across fx-115 revisions — kept at the fx-991 position and flagged for V-F. Engine still shared; **144/144 tests passing**, typecheck clean. After F-7 every model now has its own keypad-data file, so the audit trail is clean and future per-device tweaks are mechanical. |
...
| **F-9** | Refresh / add tests for any new dispatch paths. Re-run the 144-test suite and the cross-engine convergence test. | 1 hour | ✅ Shipped — added two explicit regression tests for the F-1 TI-36X Pro `pct`-key swap (primary press → `►DMS`, shifted press → `►HR`), so any future engine refactor that accidentally reverts the F-1 fix will be caught. The HYP-toggle tests already shipped with F-8 (5 cases). **Total calculator suite: 151/151 across 8 files** (was 144 at the start of the fidelity audit). Cross-engine convergence test still passes — no F-1..F-8 change affected the shared `lib/calculators/math.ts` primitives. Typecheck clean. |

### Phase 3 — Completion

| Slice | Description | Estimate |
|---|---|---|
| **V-F** | **Verification checkpoint** — capture screenshots, run all tests, document what still needs the user to spot-check against the physical hardware. | 1 hour | ✅ Shipped — **static verification**. Full calculator suite **151/151 passing** across 8 test files. Typecheck clean. All seven approved-list calculators now own dedicated keypad-data files under `lib/calculators/models/<model>/keypad-data.ts` so future per-device fidelity tweaks are mechanical. **User spot-check list for the physical hardware** (items I called out as uncertain during the audit): (1) Casio fx-991 STO/RCL row position — the real device may host STO as a SHIFT alternate on RCL rather than its own key. (2) Casio fx-991 / fx-115 natural-display fraction-builder glyph (currently `◇/▢` which is approximate). (3) Casio fx-115 `Abs` cell position — may differ by one row across fx-115 revisions. (4) HP 35s / 33s 7-column layout — emulator stays at 6 cols; the visual is close but not pixel-identical. (5) TI-36X Pro `data`/`stat-reg` and `prb` keys on the TI MultiView are visually placed but engine-no-ops; the menus they open on the real device are stat / probability editors that would each be their own future slice. (6) Casio `ENG` mode-cycling, HP 33s `→°`, TI MultiView `Un/d` mixed-number entry — all keypad-present, engine-no-op for v1. Visual confirmation against the deployed app awaits the next Vercel merge. |
| **F-10** | Move this plan to `docs/planning/completed/`. | 5 min | ✅ Shipped — plan relocated from `docs/planning/in-progress/CALCULATOR_FIDELITY.md` to `docs/planning/completed/CALCULATOR_FIDELITY.md`. All 11 slices (F-1..F-9 + V-F + F-10) annotated. Audit grew the calculator test suite from 144→151 cases and surfaced one real engine-level bug (Casio HYP toggle had a keypad but no engine handler — wired in F-8). User actions for the deployed-side spot-check are listed inline in the V-F annotation. |
