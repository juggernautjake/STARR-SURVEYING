...
| ~~2.7~~ | ~~**Homebrew feats on non-2024 characters.**~~ **ANSWERED + SHIPPED for 2014** — a saved homebrew feat is now offered at the ASI slot, uncategorised (2024's tracks do not exist in 2014). **The framing here understated the case:** 2014's OFFICIAL catalogue is *one* feat (Grappler) and can never be more — the rest is PHB-only content outside the CC-BY licence — so homebrew is not an optional extra for that edition, it is its only real feat route. PF2/IG stay excluded on purpose: they have no ASI slot, so a 5e-shaped feat there is a category error. **Still open:** surfacing the one *official* 2014 feat, which needs the system-keyed dispatcher `dnd5e-2014.ts` prescribes rather than widening the 2024-typed picker. | | | slices 76–79 |
