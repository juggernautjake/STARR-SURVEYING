...
export function useActiveTimeEntry(): {
  ...
  const { data, isLoading } = useQuery<JobTimeEntry & { _job_name: string | null }>(
...
export function useClockIn(): (params: {
  ...
  return useCallback(
    async ({ jobId, entryType }) => {
      ...
      if (!userEmail) throw new Error('Cannot clock in: no signed-in session.');
...
export function useClockOut(): () => Promise<boolean> {
  ...
  return useCallback(async () => {
    ...
    if (!open || !open.started_at) return false;
